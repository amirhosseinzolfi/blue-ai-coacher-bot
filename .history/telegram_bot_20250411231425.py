...
############################################
from langgraph_agent import compile_graph, AgentState
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    try:
        ...
        for output in graph.stream(inputs, config=config):
            ...
            if "conversation" in output and output["conversation"].get("messages"):
                final_response = output["conversation"]["messages"][-1].content
            elif "summarize_conversation" in output:
                final_response = "Conversation summarized and updated."

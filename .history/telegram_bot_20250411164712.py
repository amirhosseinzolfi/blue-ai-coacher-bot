...
async def async_summarize(messages: list, existing_summary: str, chat_id: str):
    """Asynchronously summarize conversation history"""
    ...
    try:
        ...
        # Run LLM calls in executor to avoid blocking
        summary_response = await loop.run_in_executor(
            executor,
            functools.partial(llm_summary.invoke, [HumanMessage(content=full_prompt)])
        )
        
        new_summary = summary_response.content.strip()
        logger.info(f"Background summarization completed for chat {chat_id}")
        
        # Store the summary in MongoDB (also in executor)
        await loop.run_in_executor(
            executor,
            functools.partial(
                get_business_info_collection().update_one,
                {"chat_id": chat_id},
                {"$set": {"conversation_summary": new_summary, "updated_at": datetime.datetime.utcnow()}},
                upsert=True
            )
        )
...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Non-blocking memory optimization that starts background summarization
    """
    THRESHOLD = 10
    ...
    def run_async_summarize():
        try:
            # Run in executor to avoid blocking
            future = asyncio.run_coroutine_threadsafe(
                async_summarize(conv_messages, existing_summary, chat_id),
                loop
            )
            # Add callback to handle completion
            future.add_done_callback(
                lambda f: logger.info(f"Background summarization completed for chat {chat_id}")
                if not f.exception() else 
                logger.error(f"Error in background summarization for chat {chat_id}: {f.exception()}")
            )
        except Exception as e:
            logger.error(f"Failed to start background summarization: {e}")

    # Start summarization in a separate thread
    threading.Thread(
        target=run_async_summarize,
        daemon=True,
        name=f"summarize-{chat_id}"
    ).start()
    
    # Return immediately with original state
    logger.info(f"Started background summarization for chat {chat_id}")
    return state
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    try:
        with MongoDBSaver.from_conn_string(
            ...
            # Get the latest summary from MongoDB if available
            collection = get_business_info_collection()
            summary_doc = collection.find_one({"chat_id": chat_id})
            if summary_doc and "conversation_summary" in summary_doc:
                inputs["summary"] = summary_doc["conversation_summary"]
            ...
            for output in graph.stream(inputs, config=config):
                ...
                if "agent" in output and "messages" in output["agent"]:
                    ...
                    if isinstance(most_recent_msg, AIMessage):
                        ...
                        # Return response immediately without waiting for summarization
                        if final_response:
                            break
...
# Initialize a global ThreadPoolExecutor for async operations
executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="async_summarize")

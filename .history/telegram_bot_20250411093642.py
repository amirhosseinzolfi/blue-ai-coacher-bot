...
def agent(state: AgentState):
    ...
    logger.info(f"Agent start for chat: {chat_id}")  # Minimal start log
    ai_tone = ai_tone_map.get(chat_id, "دوستانه")
    business_info = get_user_business_info(chat_id)
    messages = state["messages"]
    # ...existing code to get date, user_report...

    formatted_messages = []
    for msg in messages:
        # ...existing code to append formatted message...
        formatted_messages.append({ "role": msg.__class__.__name__.replace("Message","").lower(), "content": msg.content })
    ...
    try:
        # ...existing code to get current_shamsi_date...
        if is_multimodal:
            messages_for_llm = [
                {"role": "system", "content": prompt_template_text + (f"\nADDRESS USER: {username}" if username else "") +
            ...
            if username:
                messages_for_llm.append({
                    "role": "system",
                    "content": f"The user's name is {username}. Address them directly."
                })
            messages_for_llm.append({"role": "user", "content": f"name: {username}\nuser prompt: {prompt_input}"})
            # ----- Minimal logging for LLM invocation -----
            logger.info("LLM invoked.")
            start_time = time.time()
            response = llm.invoke(messages_for_llm)
            logger.info(f"LLM completed in {time.time() - start_time:.2f}s")
            # ------------------------------------------------
        ...
        if state["messages"] and isinstance(state["messages"][-1], AIMessage):
            if state["messages"][-1].content.strip() == ai_message.content.strip():
                ai_message.content += " (updated)"
        ...
        error_message = AIMessage(content="متأسفم، مشکلی پیش آمد.")
        new_state = state.copy()

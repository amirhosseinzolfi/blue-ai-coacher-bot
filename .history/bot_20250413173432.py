...
def new_chat(message):
    ...
    try:
        ...
        logger.info(f"New chat session created for chat '{chat_id_str}': {new_session_id}. Persistent data is preserved.")

        # Notify the user
        response_text = (
        يال    "🆕 جلسه چت جدید ایجاد شد.\n"
            "تاریخچه چت جدید آغاز گردید. اطلاعات بیزینس قبلی حفظ شده‌اند."
        )
        ...
        logger.error(f"Error creating new chat session for chat '{chat_id_str}': {e}")

...
############################################
# Import the compiled graph from langgraph_agent
############################################
from langgraph_agent import compile_graph
graph = compile_graph()
logger.info("LangGraph workflow loaded from langgraph_agent.py")

############################################
# Main Agent Function: run_agent
############################################
from langchaingraph.checkpoint.mongodb import MongoDBSaver
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    try:
        ...
        for output in graph.stream(inputs, config=config):
            ...
            if "conversation" in output and "messages" in output["conversation"]:
            ...
            elif "summarize_conversation" in output and "summary" in output["summarize_conversation"]:
                final_response = output["summarize_conversation"]["summary"]

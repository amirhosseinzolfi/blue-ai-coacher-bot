...
############################################
# Type Definitions for AgentState - IMPORTED TO langgraph_agent.py
############################################
...
class AgentState(TypedDict):
    ...
    summary: Optional[str] # Added summary to AgentState to match langgraph_agent.py


############################################
# LangGraph Workflow and Node Definitions - REMOVED and moved to langgraph_agent.py
############################################
...
#     }
#
# class MessageCounter: # IMPORTED from langgraph_agent.py
#     def __init__(self):
#         self._counters = {}
...
#             if self._counters[chat_id] >= 5:  # Summarize every 5 messages # Changed to 5 to match langgraph_agent.py
...
#             self._counters[chat_id] = 0
#
# message_counter = MessageCounter() # IMPORTED from langgraph_agent.py
#
# ############################################
# # Agent Function - REMOVED and moved to langgraph_agent.py
# ############################################
...
#         return new_state
#
# def route_tool(state: AgentState) -> str: # REMOVED and moved to langgraph_agent.py
#     logger.debug(f"Routing from agent node to optimize_memory for chat: {state['chat_id']}")
#     return "optimize_memory"
#
# # --- Workflow Initialization --- # REMOVED and moved to langgraph_agent.py
...
# workflow.add_edge("agent", "optimize_memory")
...
from langgraph_agent import app as langgraph_app # Import the app from langgraph_agent.py
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    from telegram_bot import get_session_id, save_session_id # Import session functions
    ...
    from telegram_bot import get_history_for_chat # Import history function
    ...
    inputs: AgentState = {
        ...
        "chat_id": user_id,
        "username": username,
        "summary": history_obj.summary if hasattr(history_obj, 'summary') and history_obj.summary else "" # Pass summary from history
    }

    config = {"configurable": {"thread_id": thread_id}}

    try:
        # Use the imported langgraph_app instead of the old workflow
        graph = langgraph_app # No need to compile again, it's already compiled
        logger.info(f"[bold blue]=== Main LangGraph Graph Execution Start for thread {thread_id} ===[/bold blue]")
        final_response = None
        step_count = 0
        for output in graph.stream(inputs, config=config): # Use graph.stream
            step_count += 1
...
def get_history_for_chat(telegram_chat_id: str, session_id: str = None):
    ...
    try:
        ...
        # Load summary from history object if available
        history_obj.summary = db_manager.get_conversation_summary(telegram_chat_id) # Load summary from db_manager
    except Exception as e:
...
def get_user_info(chat_id, date=None):
    ...
    # client_db = MongoClient(MONGO_CONNECTION_STRING) # No need to create new client, use db_manager
    # db = client_db[DATABASE_NAME]
    # USER_INFO_COLLECTION = db["user_info"]
    # doc = USER_INFO_COLLECTION.find_one({"chat_id": chat_id, "date": today})
    user_info_data = db_manager.get_setting(chat_id, "user_info") # Use db_manager to get user_info
    if user_info_data and isinstance(user_info_data, dict) and user_info_data.get("date") == today:
        return user_info_data.get("info", "")
    return "" #doc["user_info"] if doc and "user_info" in doc else "" # Modified to use db_manager
...
def save_message_to_history(chat_id, role, content):
    try:
        ...
        # Save conversation summary to db_manager after each message
        conversation_summary = "" # You might want to re-summarize here or just store the flag that summarization is needed. For now, skip auto-summarization after each message save to avoid performance hit.
        # db_manager.save_conversation_summary(chat_id, conversation_summary) # Save summary using db_manager. For now, skip auto-summarization to avoid performance hit.
    except Exception as e:
...
# Exports
############################################
from message_counter import MessageCounter, message_counter
...
__all__ = [
    ...
    "daily_users_report",
    "load_user_reports",
    "message_counter", # Export message_counter remains imported from message_counter module
]
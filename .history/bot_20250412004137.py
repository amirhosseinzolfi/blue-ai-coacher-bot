...
def new_chat(message):
    ...
    save_business_info(int_chat_id, {"user_report": ""}, chat_type=message.chat.type)
...
def send_welcome(message):
    ...
    save_message_to_history(chat_id, "system", welcome_message, chat_type=message.chat.type)
...
def send_help(message):
    ...
    from db_manager import save_message_to_history
    save_message_to_history(str(message.chat.id), "system", help_text_prompt, chat_type=message.chat.type)
...
def about_bot(message):
    ...
    from db_manager import save_message_to_history
    save_message_to_history(str(message.chat.id), "system", about_text, chat_type=message.chat.type)
...
def bot_settings(message):
    ...
    from db_manager import save_message_to_history
    save_message_to_history(chat_id, "system", settings_text, chat_type=message.chat.type)
...
def options_handler(message):
    ...
    from db_manager import save_message_to_history
    save_message_to_history(chat_id, "system", options_text, chat_type=message.chat.type)
...
def handle_message(message):
    ...
    if business_info_update_pending.get(chat_id):
        ...
        save_user_business_info(chat_id, optimized, chat_type=message.chat.type)
    ...
    from db_manager import save_message_to_history
    save_message_to_history(chat_id, "user", f"{sender_first_name}: {user_message_text}", chat_type=chat_type)
    ...
    try:
        ...
        save_message_to_history(chat_id, "assistant", refined_response, chat_type=chat_type)
...
def handle_document(message):
    ...
    if business_info_update_pending.get(chat_id):
        ...
        if mode == "append":
            ...
            optimized = new_info
        from db_manager import save_user_business_info  # or save_business_info if shared
        save_user_business_info(chat_id, optimized, chat_type=message.chat.type)
        business_info_update_pending.pop(chat_id, None)
        business_info_mode.pop(chat_id, None)
        bot.send_message(chat_id, "✅ اطلاعات بیزینس به‌روزرسانی شد.")

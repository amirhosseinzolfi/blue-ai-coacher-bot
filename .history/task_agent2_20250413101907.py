...
def run_agent(state: AgentState) -> Dict[str, Union[str, List[Dict[str, str]]]]:
    ...
    if "Action:" in response and "Action Input:" in response:
        ...
        # Check for keywords to improve task identification
        if "add" in user_input:
            task_description = user_input.replace("add", "").strip()
            if task_description:
                tool_output = add_task(task_description)
                return {"action": "add_task", "action_input": task_description, "tool_output": tool_output, "tasks": get_current_tasks()}
            else:
                return {"response": "Please specify the task to add.", "tasks": get_current_tasks()}
        elif "complete" in user_input:
            try:
                task_number = int(user_input.split("complete task:")[1].strip())
                tool_output = complete_task(task_number)
                return {"action": "complete_task", "action_input": str(task_number), "tool_output": tool_output, "tasks": get_current_tasks()}
            except ValueError:
                return {"response": "Invalid task number. Please specify the number of the task to complete.", "tasks": get_current_tasks()}
        elif "refine" in user_input:
            tool_output = refine_tasks(user_input, conversation_history)
            return {"action": "refine_tasks", "action_input": user_input, "tool_output": tool_output, "tasks": get_current_tasks()}
        elif "list" in user_input:
             tool_output = list_tasks()
             return {"action": "list_tasks", "action_input": "", "tool_output": tool_output, "tasks": get_current_tasks()}
        else:
            return {"response": response, "tasks": get_current_tasks()}  # Return the LLM response
        #return {"response": response, "tasks": get_current_tasks()}  # Return the LLM response

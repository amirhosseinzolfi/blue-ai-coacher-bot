...
# Replace global instance creation and exported helper functions with lazy initialization
_db_manager = None
...
# Exported helper functions updated to use lazy initialization
def save_business_info(chat_id: str, info: str, chat_type: str = "private") -> None:
    get_db_manager().save_business_info(chat_id, info, chat_type)

def get_business_info(chat_id: str) -> str:
    return get_db_manager().get_business_info(chat_id, "private")

def save_user_info(chat_id: str, info: str, date: Optional[str] = None) -> None:
    today = date or datetime.datetime.now().strftime("%Y-%m-%d")
    get_db_manager().save_setting(chat_id, "user_info", {"info": info, "date": today})

def get_user_info(chat_id: str, date: Optional[str] = None) -> str:
    setting = get_db_manager().get_setting(chat_id, "user_info")
...
def save_message_to_history(chat_id: str, role: str, content: str, session_id: Optional[str] = None) -> None:
    get_db_manager().save_chat_message(chat_id, role, content, session_id or f"{chat_id}_default")

def get_chat_history(chat_id: str, limit: int = 50) -> List[Dict]:
    return get_db_manager().get_chat_history(chat_id, "private", limit=limit)

def load_chat_data(chat_id: str, chat_type: str = "private") -> Dict:
    return get_db_manager().load_all_chat_data(chat_id, chat_type)

def save_ai_tone(chat_id: str, tone: str, chat_type: str = "private") -> None:
    get_db_manager().save_ai_tone(chat_id, tone, chat_type)

def get_ai_tone(chat_id: str, chat_type: str = "private") -> str:
    return get_db_manager().get_ai_tone(chat_id, chat_type)

# Update __all__ for exported symbols
__all__ = [
    'DatabaseManager',
    'get_db_manager',
    'save_business_info',

...
def agent(state: AgentState):
    ...
    try:
        ...
        # Remove direct optimize_memory call, let the route_tool handle it
        return new_state
    except Exception as e:
...
def route_tool(state: AgentState) -> str:
    """
    Route to optimize_memory only every 10 messages.
    """
    chat_id = state["chat_id"]
    if message_counter.increment_and_check(chat_id):
        logger.info(f"[bold blue]Message count threshold reached for chat {chat_id}, routing to optimize_memory[/bold blue]")
...
workflow.add_edge("route_tool", "optimize_memory")

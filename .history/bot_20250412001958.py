...
def new_chat(message):
    ...
    # Save the new session metadata permanently and clear conversation history
    from db_manager import db_manager
    db_manager.clear_chat_history(chat_id_str)
    db_manager.save_chat_metadata(chat_id_str, "private", {
        "session_id": new_session_id,
        "created_at": datetime.datetime.utcnow()
    })
    logger.info(f"New chat session created for chat '{chat_id_str}': {new_session_id} (Chat history reset)")

    response_text = "🆕 جلسه چت جدید ایجاد شده و تاریخچه چت بازنشانی شد."
...
def handle_load_business_info(call):
    chat_id = str(call.message.chat.id)
    from db_manager import db_manager
    current_info = db_manager.get_business_info(chat_id)
    
    # Debug logging
    logger.info(f"[bold blue]Loading business info for chat {chat_id}[/bold blue]")
    logger.info(f"Current info exists: {bool(current_info)}")
    ...
    if current_info:
        ...
        # Log retrieved info
        logger.info(f"[bold green]Retrieved business info:[/bold green] {current_info[:100]}...")
    else:
...
def handle_message(message):
    ...
    if business_info_update_pending.get(chat_id):
        ...
        logger.info(f"[bold blue]Saving business info for chat {chat_id}[/bold blue]")
        db_manager.save_business_info(chat_id, optimized)
        business_info_update_pending.pop(chat_id, None)
        business_info_mode.pop(chat_id, None)
        
        # Verify save
        saved_info = db_manager.get_business_info(chat_id)
        logger.info(f"[bold green]Verified saved business info:[/bold green] {bool(saved_info)}")

...
class DatabaseManager:
    ...
    def save_setting(self, chat_id: str, setting_type: str, value: any, chat_type: str = "private") -> None:
        """Save or update a user/chat setting (such as AI tone, user info, or session ID)."""
        try:
            self._ensure_connection()
            ...
            logging.error(f"Error saving setting '{setting_type}' for chat '{chat_id}': {e}")
            raise
    ...
    def get_setting(self, chat_id: str, setting_type: str) -> Optional[any]:
        ...
        try:
            ...
            logging.error(f"Error retrieving setting '{setting_type}' for chat '{chat_id}': {e}")
            return None

...
_set_env("ANTHROPIC_API_KEY")  # Prompt for Anthropic key if not set
...
def extract_semantic_memory(messages):
    try:
        # Convert messages to dicts for LangMem
        input_messages = []
        for m in messages:
        ...
        # Try using the same LLM model we already have configured
        semantic_manager = create_memory_manager(
            model=model,  # Use our existing configured model instead of Anthropic
            schemas=[Triple],

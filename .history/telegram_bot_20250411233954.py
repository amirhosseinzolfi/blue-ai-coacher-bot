...
class State(MessagesState):
    # New state extends MessagesState and carries a summary.
    summary: str
...
def call_model(state: State):
    ...
    if summary:
        system_msg = SystemMessage(content=f"Summary of conversation earlier: {summary}")
...
def should_continue(state: State) -> Union[Literal["summarize_conversation"], Literal[END]]:
    # When enough messages are accumulated, trigger summarization.
    msgs = state["messages"]
    if len(msgs) > 6:
...
def summarize_conversation(state: State):
    ...
    if summary:
        ...
        prompt_text = "Create a summary of the conversation above:"
    messages = state["messages"] + [HumanMessage(content=prompt_text)]
    response = llm.invoke(messages)
    # Remove older messages except for the last two
    delete_msgs = [RemoveMessage(id=m.id) for m in state["messages"][:-2] if hasattr(m, "id")]
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    try:
        with MongoDBSaver.from_conn_string(
            ...
            new_graph = workflow.compile(checkpointer=checkpointer)
            logger.info(f"[bold blue]=== New LangGraph Graph Execution Start for chat {chat_id} ===[/bold blue]")
            final_response = None
            for update in new_graph.stream({"messages": [HumanMessage(content=query)]}, config=config, stream_mode="updates"):
                if "summarize_conversation" in update and "summary" in update["summarize_conversation"]:
                    final_response = update["summarize_conversation"]["summary"]
                elif "conversation" in update and "messages" in update["conversation"]:
                    # Expect the AIMessage response from the conversation node.
                    for m in update["conversation"]["messages"]:
                        if isinstance(m, AIMessage):
                            final_response = m.content
            if not final_response or not final_response.strip():
                final_response = "متأسفم، نتوانستم پاسخ مناسبی بیابم. لطفا دوباره تلاش کنید."
            # Optionally refine the response as before.
            from utils.helpers import refine_ai_response

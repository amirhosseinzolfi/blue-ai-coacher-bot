...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Summarize the conversation if threshold exceeded and prune messages.
    Keeps a summary and the most recent N messages for efficiency.
    """
    THRESHOLD = 10  # Adjust threshold based on LLM context limits
    KEEP_RECENT = 2  # Number of latest messages to retain
    messages = state["messages"]
    
    if len(messages) <= THRESHOLD:
        logger.info(f"Message count ({len(messages)}) within threshold ({THRESHOLD}); skipping summarization.")
        return state

    # Extract previous summary if present
    previous_summary = state.get("summary", "")
    if messages and isinstance(messages[0], SystemMessage) and messages[0].content.startswith("[CONVERSATION SUMMARY]:"):
        previous_summary = messages[0].content.replace("[CONVERSATION SUMMARY]:", "").strip()
        messages = messages[1:]  # Exclude the summary from further processing

    # Generate conversation text for summarization
    conversation_text = "\n".join(f"{msg.type}: {msg.content}" for msg in messages)

    prompt_input = SUMMARY_PROMPT_TEXT.format(
    ...
    try:
        response = llm_summary.invoke([HumanMessage(content=prompt_input)])
        new_summary = response.content.strip()
        logger.info(f"Summarization completed: {new_summary[:100]}...")
        
        # Keep new summary and last KEEP_RECENT messages
        new_system_message = SystemMessage(content=f"[CONVERSATION SUMMARY]: {new_summary}")
        updated_messages = [new_system_message] + messages[-KEEP_RECENT:]
...
class MessageCounter:
    ...
    def increment_and_check(self, chat_id: str) -> bool:
        with self._lock:
            ...
            if self._counters[chat_id] >= self._threshold:

...
def new_chat(message):
    ...
    # Create new session and get its ID
    new_session_id = new_chat_session(chat_id)
    
    response_text = (
        "🆕 *جلسه چت جدید ایجاد شد*\n\n"
        "• تاریخچه چت جدید آغاز شد\n"
        "• اطلاعات بیزینس و تنظیمات شما حفظ شده‌اند\n"
        "• می‌توانید گفتگوی جدیدی را شروع کنید"
    )
    
    bot.reply_to(message, escape_markdown_v2(response_text), parse_mode="MarkdownV2")
    logger.info(f"Created new chat session '{new_session_id}' for chat '{chat_id}'")

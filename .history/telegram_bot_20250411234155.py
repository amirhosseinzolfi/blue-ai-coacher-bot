...
All core functionalities remain unchanged except for the LangGraph core.
"""
...
############################################
# Type Definitions for AgentState (Legacy, now unused by new graph)
############################################
...
############################################
# New LangGraph Workflow Core Integration
############################################
# Replace the older graph (agent & optimize_memory) with a new graph that
# uses the latest LangGraph structure.

from typing import Literal, Union  # new types
from langgraph.graph import MessagesState, StateGraph, START, END
from langchain.schema import SystemMessage, HumanMessage, AIMessage  # our standard message types

# Define our new state extending the base messages state with a summary field.
class AppState(MessagesState):
    summary: str

def call_model(state: AppState):
    """
    Prepend any existing conversation summary as a system message and invoke the primary LLM.
    """
    summary = state.get("summary", "")
    if summary:
        sys_msg = SystemMessage(content=f"Summary of conversation earlier: {summary}")
...
def should_continue(state: AppState) -> Union[Literal["summarize_conversation"], Literal[END]]:
    """
    If the conversation has grown too long, trigger summarization.
    """
...
def summarize_conversation(state: AppState):
    """
    Create or extend a conversation summary and trim conversation history.
    """
    current_summary = state.get("summary", "")
    if current_summary:
        prompt_text = f"Extend the previous summary: {current_summary}\n\nAnd summarize the new conversation messages:"
    else:
        prompt_text = "Please create a summary of the conversation above:"
    
    summary_prompt_message = HumanMessage(content=prompt_text)
    response = llm_summary.invoke([summary_prompt_message])
    new_summary = response.content.strip()
    # Keep only the last two messages to reduce context size.
    new_messages = state["messages"][-2:]
    return {"summary": new_summary, "messages": new_messages}

# Build the new LangGraph workflow using our AppState.
new_workflow = StateGraph(AppState)
logger.info("New LangGraph workflow initialized with AppState structure")
new_workflow.add_node("conversation", call_model)
new_workflow.add_node("summarize_conversation", summarize_conversation)
new_workflow.add_edge(START, "conversation")
new_workflow.add_conditional_edges("conversation", should_continue, ["summarize_conversation", END])
new_workflow.add_edge("summarize_conversation", END)
logger.info("New LangGraph workflow nodes and edges defined")

############################################
# Thread-Safe Message Counting (unchanged)
############################################
...
############################################
# Agent Function (legacy, for multimodal handling etc. remains for other parts)
############################################
...
############################################
# Updated run_agent with New Workflow
############################################
...
def run_agent(query, chat_id, message_id, username=None):
    """
    Processes a user query using the new LangGraph workflow with MongoDB checkpointer integration.
    ...
    session_id = chat_session_map.get(chat_id, None)
    if not session_id:
    ...
    # Build initial state for the new workflow.
    inputs = {
        "messages": history_messages + [human_message],
        "summary": "",   # Start with empty summary; will be updated by the graph.
        "chat_id": user_id,
        "username": username,
        "tool_calls": [],
        "requires_tool": False,
    ...
    try:
        with MongoDBSaver.from_conn_string(
            ...
            graph = new_workflow.compile(checkpointer=checkpointer)
            ...
            for output in graph.stream(inputs, config=config):
                ...
                if "summarize_conversation" in output and "messages" in output["summarize_conversation"]:
                    most_recent_msg = output["summarize_conversation"]["messages"][-1]
                    if isinstance(most_recent_msg, AIMessage):
                        final_response = most_recent_msg.content
                        logger.debug("Got response from summarize_conversation node")
                elif "conversation" in output and "messages" in output["conversation"] and final_response is None:
                    most_recent_msg = output["conversation"]["messages"][-1]
                    if isinstance(most_recent_msg, AIMessage):
                        final_response = most_recent_msg.content
                        logger.debug("Got response from conversation node")
...
############################################
# MongoDB Helper Functions for Chat History & Business Info (unchanged)
############################################
...
############################################
# Business Info Summarization and Processing (unchanged)
############################################
from langchain.chains.summarize import load_summarize_chain

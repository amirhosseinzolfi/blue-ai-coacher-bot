...
# Define the agent function without decorator
def agent(state: AgentState[AgentStateData]) -> AgentState[AgentStateData]:
    """
    Enhanced agent node with better error handling and state management
    """
    ...
    try:
        messages_for_llm = create_messages_for_llm(state)
        response = llm.invoke(messages_for_llm)
        
        new_state = state.copy()
        new_state.data.messages.append(AIMessage(content=response.content))
...
def optimize_memory(state: AgentState[AgentStateData]) -> AgentState[AgentStateData]:
    """
    ...
    THRESHOLD = 10
    conv_messages = [msg for msg in state.data.messages if isinstance(msg, (HumanMessage, AIMessage))]
    ...
    existing_summary = state.data.get("summary", "")
    ...
    state.data["summary"] = new_summary
    return {
        "messages": new_messages,
        "tool_calls": state.data.get("tool_calls", []),
        "requires_tool": state.data.get("requires_tool", False),
        "current_tool": state.data.get("current_tool", None),
        "chat_id": state.data["chat_id"],
        "username": state.data.get("username", None),
        "summary": new_summary,
    }

# Initialize workflow with proper typing
workflow = StateGraph(AgentState[AgentStateData])
...
def create_messages_for_llm(state: AgentState[AgentStateData]) -> List[Dict]:
    """Creates formatted messages for LLM from state"""
    messages = []
...
def should_summarize(state: AgentState[AgentStateData]) -> bool:
    """Determines if the conversation needs summarization"""
    return len(state.data.messages) >= 10

def handle_agent_error(state: AgentState[AgentStateData]) -> AgentState[AgentStateData]:
    ...
    new_state.data.messages.append(error_message)
    return new_state
...
# Define better state management
...
# Better state management with TypeVar
S = TypeVar('S', bound=AgentStateData)
...
def agent(state: AgentState[AgentStateData]) -> AgentState[AgentStateData]:
    ...
    try:
        ...
        if state.data.messages and isinstance(state.data.messages[-1], AIMessage):
            last_response = state.data.messages[-1].content.strip()
        ...
        new_state = state.copy()
        new_state.data.messages.append(ai_message)

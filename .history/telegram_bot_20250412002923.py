...
def agent(state: AgentState):
    ...
    # NEW: Assign messages from the state to fix NameError
    messages = state["messages"]
    
    # Load context with error handling
    try:
        from db_manager import db_manager
        context = db_manager.load_chat_context(chat_id, "private")
        business_info = context.get("business_info", "")
        ai_tone = context.get("settings", {}).get("ai_tone", "دوستانه")
        
        logger.info(f"[bold cyan]Loaded context for chat {chat_id}:[/bold cyan]")
        logger.info(f"Business info present: {bool(business_info)}")
        logger.info(f"Using AI tone: {ai_tone}")
    ...
    # Format system prompt with business info
    try:
        system_prompt = f"""
        {prompt_template_text}

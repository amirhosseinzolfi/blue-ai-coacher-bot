...
USER_REPORT_LLM_MODEL = "gemini-1.5-flash"
...
# Add missing atexit registrations to close all LLM client sessions
atexit.register(lambda: user_llm.client.close() if hasattr(user_llm, "client") and callable(getattr(user_llm, "client", None)) else None)
atexit.register(lambda: llm_summary.client.close() if hasattr(llm_summary, "client") and callable(getattr(llm_summary, "client", None)) else None)
...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Summarize conversation if triggered by user message count.
    ...
    logger.process_start(f"[bold yellow]Starting conversation optimization for chat {chat_id}[/bold yellow]")
    logger.info(f"Current message count: {len(conv_messages)}")
    
    if not conv_messages:
        logger.info("[cyan]No messages to summarize[/cyan]")
        return state
    ...
    try:
        ...
        logger.info("[bold blue]Starting conversation summarization...[/bold blue]")
        ...
        logger.info(f"[green]✅ Summarization completed in {duration:.2f}s[/green]")
        log_summarization(logger, conversation_text, new_summary, "Conversation")
        # New: Log the final summary result for terminal visibility
        logger.info(f"[bold cyan]Conversation Summary:[/bold cyan] {new_summary}")
    except Exception as e:
        logger.error(f"[bold red]Error during summarization: {e}[/bold red]", exc_info=True)
        new_summary = "Summary not available due to an error."
        logger.process_end("[red]Summarization failed, using fallback summary[/red]")
        return state

    new_system = SystemMessage(content=f"[CONVERSATION SUMMARY]: {new_summary}")
    new_messages = [new_system] + conv_messages[-2:]
    
    logger.info(f"[blue]Trimmed conversation to {len(new_messages)} messages (summary + last 2)[/blue]")
    logger.process_end("[green]Conversation optimization completed successfully[/green]")
...
class MessageCounter:
    ...
    def increment_and_check(self, chat_id: str, is_user_message: bool = False) -> bool:
        if not is_user_message:  # Only count user messages
            return False
            
        with self._lock:
            self._user_message_counters[chat_id] = self._user_message_counters.get(chat_id, 0) + 1
            count = self._user_message_counters[chat_id]
            logger.info(f"[cyan]User message count for chat {chat_id}: {count}/10[/cyan]")
            
            if count >= 10:
                logger.info(f"[yellow]🔄 Triggering summarization for chat {chat_id} after {count} user messages[/yellow]")
                self._user_message_counters[chat_id] = 0  # Reset counter
                return True
            return False

    def reset(self, chat_id: str):
        with self._lock:
            self._user_message_counters[chat_id] = 0
            logger.info(f"[cyan]Reset message counter for chat {chat_id}[/cyan]")
...
def agent(state: AgentState):
    ...
    try:
        ...
        if message_counter.increment_and_check(chat_id, is_user_message=True):
...
MONGODB_TIMEOUT = 5  # seconds
SESSION_CACHE_TTL = 300  # seconds

# Add session caching
from cachetools import TTLCache
session_cache = TTLCache(maxsize=100, ttl=SESSION_CACHE_TTL)

# Remove the signal-based timeout implementation
# Add new timeout implementation using asyncio
import asyncio
from concurrent.futures import ThreadPoolExecutor

async def async_timeout(coroutine, timeout_seconds):
    try:
...
def run_with_timeout(func, *args, timeout_seconds=30):
    """Run a function with timeout using asyncio"""
    async def wrapper():
    ...
    try:
        ...
        result = loop.run_until_complete(async_timeout(wrapper(), timeout_seconds))
        loop.close()
        return result
    except TimeoutError:
        raise
...
def run_agent(query, chat_id, message_id, username=None):
    """
    Processes a user query using LangGraph with optimized performance.
    """
    ...
    cache_key = f"{chat_id}_session"
    
    try:
        # Try to get session from cache first
        if cache_key in session_cache:
            session_id = session_cache[cache_key]
            logger.debug(f"Session ID retrieved from cache for chat {chat_id}")
        else:
            session_id = get_session_id(chat_id)
            if session_id:
                session_cache[cache_key] = session_id
                logger.debug(f"Session ID cached for chat {chat_id}")
        ...
        history_obj = get_history_for_chat(chat_id, session_id, timeout=MONGODB_TIMEOUT)
        ...
        # Format query with timeout handling
        formatted_query = format_query(query, username)
        human_message = create_human_message(formatted_query, is_multimodal)
        
        state_messages = history_messages + [human_message]
        inputs = create_agent_state(state_messages, chat_id, username)
        ...
        def process_workflow():
            with MongoDBSaver.from_conn_string(
                ...
                final_response = None
                for output in graph.stream(inputs, config={"configurable": {"thread_id": session_id}}):
        ...
        final_response = run_with_timeout(process_workflow, timeout_seconds=LLM_TIMEOUT)
        ...
        logger.error(f"❌ Timeout after {LLM_TIMEOUT}s waiting for response")
        return "متأسفم، پردازش درخواست شما بیش از حد طول کشید. لطفاً دوباره تلاش کنید."
    except Exception as e:
        logger.error(f"❌ Error during LangGraph execution: {e}", exc_info=True)
        return "مشکلی در پردازش درخواست شما پیش آمد. لطفاً دوباره تلاش کنید."
...
def format_query(query, username):
    """Format query with optimized string handling"""
    if isinstance(query, list):
        return query
    return f"name : {username}\n# user prompt : {query}\n\n"

def create_human_message(formatted_query, is_multimodal):
    """Create appropriate message object based on query type"""
    from langchain.schema import HumanMessage
    return HumanMessage(content=formatted_query)

def create_agent_state(messages, chat_id, username):
    """Create agent state with required fields"""
    return {
        "messages": messages,
        "tool_calls": [],
        "requires_tool": False,
...
def process_agent_workflow(inputs, config):
    """Process workflow with proper error handling"""
    try:
        with MongoDBSaver.from_conn_string(
            MONGO_CONNECTION_STRING,
            db_name=DATABASE_NAME,
            ...
            for output in graph.stream(inputs, config=config):
                if "agent" in output and "messages" in output["agent"]:
                    most_recent_msg = output["agent"]["messages"][-1]
                    if isinstance(most_recent_msg, AIMessage):
                        return most_recent_msg.content
    except Exception as e:
...
def get_history_for_chat(telegram_chat_id: str, session_id: str = None, timeout: int = MONGODB_TIMEOUT):
    if not session_id:

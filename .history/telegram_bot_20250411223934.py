...
def run_agent(query, chat_id, message_id, username=None):
    """
    Process a query using the new LangGraph implementation.
    """
    from langgraph-agent import process_chat_update
    
    # Get business info and AI tone
    business_info = get_user_business_info(chat_id)
    from config import ai_tone_map
    ai_tone = ai_tone_map.get(chat_id, "دوستانه")
    
    # Process the update
    result = process_chat_update(
    ...
    if "error" in result:
        logger.error(f"Error in LangGraph processing: {result['error']}")
        return "متأسفم، مشکلی در پردازش درخواست شما پیش آمد."
        
    # Refine and return the response
    from utils.helpers import refine_ai_response
    return refine_ai_response(result["response"])

        ...
        while True:
            ...
            try:
                ...
                if hasattr(response, 'content'):
                    ...
                    response_id = metadata.get('id', 'unknown')
                ...
                # Create structured response table
                resp_table = Table(show_header=False, box="ROUNDED", expand=True)
                resp_table.add_column("Field", style="bold cyan", width=15)
                resp_table.add_column("Value", style="green")

                # Add rows with formatted content
                resp_table.add_row("Content", f"\n{content}\n")
                resp_table.add_row("Model", model)
                resp_table.add_row("Response ID", response_id)
                resp_table.add_row("Status", f"[bright_green]✓ {finish_reason}[/bright_green]")
                resp_table.add_row("Tokens", f"[yellow]Used: {completion_tokens} | Total: {total_tokens}[/yellow]")

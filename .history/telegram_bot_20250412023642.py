...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Summarize conversation if there are more than 6 human/AI messages.
    Then keep only an updated summary and the last 2 messages.
    """
    ...
    # Check for an existing summary and set an instruction to extend it
    if conv_messages and isinstance(conv_messages[0], SystemMessage) and conv_messages[0].content.startswith("[CONVERSATION SUMMARY]:"):
        ...
        summary_instruction = f"This is the current summary:\n{previous_summary}\n\nPlease extend the summary by including new conversation details."
        new_messages_for_summary = conv_messages[1:]
    else:
        previous_summary = ""
        summary_instruction = "Please create a concise summary of the conversation so far."
        new_messages_for_summary = conv_messages

    conversation_text = "\n".join([f"{msg.__class__.__name__}: {msg.content}" for msg in new_messages_for_summary])
    ...
    # Create a more explicit summary message for the LLM context
...
def agent(state: AgentState):
    ...
    try:
        if is_multimodal:
            ...
            # Prepend the stored conversation summary if it exists.
            if state.get("summary"):
                messages_for_llm.insert(0, {"role": "system", "content": f"Conversation Summary: {state['summary']}\nPlease use this context for your answer."})
            for msg in formatted_messages[:-1]:
                messages_for_llm.append(msg)
            if username:

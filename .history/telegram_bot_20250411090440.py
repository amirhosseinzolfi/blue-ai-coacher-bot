...
All core functionalities remain unchanged except for the conversation
summarization method, which now matches the provided guide.
"""
...
# ====================================================
# New Conversation Summarization Function (Synchronous)
# ====================================================
def optimize_memory(state: AgentState) -> AgentState:
    """
    Summarize conversation history if there are 10 or more conversation messages.
    
    This function:
      1. Checks if there are 10 or more messages (counting only HumanMessage and AIMessage).
      2. If yes, creates a summary using a prompt (extending any existing summary if available).
      3. Trims the conversation history to keep only the summary (as a system message) 
         and the last two conversation messages.
    """
    THRESHOLD = 10
    chat_id = state["chat_id"]
    conv_messages = [msg for msg in state["messages"] if isinstance(msg, (HumanMessage, AIMessage))]
    logger.info(f"Chat '{chat_id}': found {len(conv_messages)} conversation messages.")
    
    if len(conv_messages) < THRESHOLD:
        logger.info("Message count below threshold; no summarization triggered.")
        return state

    existing_summary = state.get("summary", "")
    if existing_summary:
        summary_prompt_text = (
            f"This is the summary of our conversation so far: {existing_summary}\n\n"
            "Extend this summary by incorporating the new messages below:"
        )
    else:
        summary_prompt_text = "Create a summary of the conversation above:"

    conversation_text = "\n".join(
        [f"{msg.__class__.__name__}: {msg.content}" for msg in conv_messages]
    )
    full_prompt = f"{conversation_text}\n\n{summary_prompt_text}"
    logger.info("Sending summarization request to the LLM.")
    ...
    # Create a system message with the new summary.
    summary_system_msg = SystemMessage(content=f"[CONVERSATION SUMMARY]: {new_summary}")
    # Retain only the last two messages to preserve recent context.
    trimmed_messages = state["messages"][-2:]
    new_messages = [summary_system_msg] + trimmed_messages
    logger.info(f"Conversation history trimmed for chat '{chat_id}': kept summary and last 2 messages.")
    ...
    return new_state
# ====================================================
# End of Conversation Summarization Function
# ====================================================

# (Background summarization routines removed as per the new guide)
    ...
    try:
        ...
        if username:
            for placeholder in ["[نام کاربر]", "[name]", "[نام]"]:
                response.content = response.content.replace(placeholder, username)
        from utils.helpers import strip_thinking_tags
        response_content = strip_thinking_tags(response.content)
        ...
        # Run conversation summarization synchronously (using our new method)
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    logger.info(f"Starting LangGraph agent for chat: {chat_id}")
    start_time = time.time()
...
def summarize_business_info(raw_text: str) -> str:
    try:
        logger.info("Summarizing business info...")
        ...
        logger.info(f"Business info summarized in {duration:.2f}s")
        return result
    except Exception as e:
    ...
    try:
        ...
        for report in reports:
            if "chat_id" in report and "user_report" in report:
                ...
                count += 1
        logger.info(f"Loaded {count} user reports from database")
    except Exception as e:

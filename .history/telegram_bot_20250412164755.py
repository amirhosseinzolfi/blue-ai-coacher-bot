...
def run_agent(query, chat_id, message_id, username=None):
    ...
    from db_manager import get_conversation_summary
    conversation_summary = get_conversation_summary(user_id, session_id)
...
def save_message_to_history(chat_id: str, role: str, content: str, session_id: Optional[str] = None) -> None:
    ...
    Passes session_id to the db_manager helper. If no session_id is provided,
    uses the default format.
    """
    from db_manager import save_message_to_history as db_save_msg
    db_save_msg(chat_id, role, content, session_id)
...
def new_chat_session(chat_id: str) -> str:
    ...
    Creates a new chat session for the given chat_id:
      ...
      - Saves the new session ID permanently (without deleting previous session data).
    Returns the new session ID.
    """
    new_session_id = f"{chat_id}_{int(datetime.datetime.now().timestamp())}"
    
    # Save the new session ID using the db_manager helper.
    from db_manager import save_session_id
    save_session_id(chat_id, new_session_id)
    
    logger.info("New chat session '%s' started for chat '%s'. All previous sessions are preserved.", new_session_id, chat_id)

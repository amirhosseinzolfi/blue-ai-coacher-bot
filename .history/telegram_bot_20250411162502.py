...
def run_agent(query, chat_id, message_id, username=None, chat_type: str = "private"):
    """
    ...
    session_id = get_session_id(user_id, chat_type)
    if not session_id:
        session_id = f"{user_id}_{int(datetime.datetime.now().timestamp())}"
        save_session_id(user_id, session_id, chat_type)
        logger.info(f"Created new session '{session_id}' for chat '{user_id}' with type '{chat_type}'.")
    
    thread_id = session_id
    logger.info(f"🧵 Using thread ID: {thread_id}")
    
    logger.debug("Loading conversation history from MongoDB")
    history_obj = get_history_for_chat(user_id, session_id, chat_type)
...
def get_user_business_info(chat_id: str, chat_type: str = "private") -> str:
    collection = get_business_info_collection()
    result = collection.find_one({"chat_id": chat_id, "chat_type": chat_type})
...
def save_user_business_info(chat_id: str, info: str, chat_type: str = "private"):
    ...
    logger.info("Updated business info for chat '%s' (type: %s)", chat_id, chat_type)
...
def get_history_for_chat(telegram_chat_id: str, session_id: str = None, chat_type: str = "private"):
    if not session_id:
        session_id = get_session_id(telegram_chat_id, chat_type)
        if not session_id:
            session_id = f"{telegram_chat_id}_{int(datetime.datetime.now().timestamp())}"
            save_session_id(telegram_chat_id, session_id, chat_type)
            logger.info(f"Created new session '{session_id}' for chat '{telegram_chat_id}' with type '{chat_type}'.")
    logger.debug(f"Retrieving history for session '{session_id}' in chat '{telegram_chat_id}' (type: {chat_type}).")
    from langchain_mongodb.chat_message_histories import MongoDBChatMessageHistory
    history_obj = MongoDBChatMessageHistory(
        session_id=session_id,
        connection_string=MONGO_CONNECTION_STRING,
...
def save_session_id(chat_id: str, session_id: str, chat_type: str = "private"):
    collection = get_business_info_collection()
    collection.update_one(
        {"chat_id": chat_id, "chat_type": chat_type},
    ...
    logger.info(f"Saved session ID '{session_id}' for chat '{chat_id}' with type '{chat_type}'.")

def get_session_id(chat_id: str, chat_type: str = "private") -> Optional[str]:
    collection = get_business_info_collection()
    result = collection.find_one({"chat_id": chat_id, "chat_type": chat_type})

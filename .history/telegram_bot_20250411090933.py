...
and the G4F API (LLM-based responses).

Note: This file has been refactored for improved readability and structure.
All core functionalities are preserved except that conversation history
summarization is now done synchronously (per the provided guide).
"""
...
def new_chat(message):
    ...
    from telegram_bot import save_session_id
...
# (Handlers for daily_tasks, instagram_story_idea, chat_report, shamsi_date, load_business_info, and ai_tone follow here.)
# These remain unchanged and are omitted for brevity.
# ...
...
def handle_message(message):
    ...
    # Check for AI Tone update or Business Info update messages (handled separately)
    ...
    if message.content_type == 'photo':
        ...
        logger.info(f"Photo received from {sender_first_name} in chat {chat_id} with caption: {text_component}")
    else:
        query_payload = message.text
        user_message_text = message.text

    logger.info(f"Message from {sender_first_name} in chat {chat_id}: {user_message_text[:50]}{'...' if len(user_message_text) > 50 else ''}")
    ...
    if chat_type in ['group', 'supergroup']:
        ...
        if not is_mentioned:
            logger.debug(f"Message in group {chat_id} not mentioning bot - no response")
            return
    ...
    try:
        # Call the LangGraph agent (which internally applies the synchronous summarization)
        ...
        logger.info(f"✅ Response sent to {sender_first_name}")

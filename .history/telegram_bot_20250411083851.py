...
def optimize_memory(state: AgentState) -> AgentState:
    ...
    if existing_summary:
        summary_prompt_text = f"This is the summary of our conversation so far: {existing_summary}\n\nExtend the summary by incorporating the new messages below:"
    else:
        summary_prompt_text = "Create a summary of the conversation above:"
    conversation_text = "\n".join([f"{msg.__class__.__name__}: {msg.content}" for msg in conv_messages])
    ...
    new_summary_msg = SystemMessage(content=f"[CONVERSATION SUMMARY]: {new_summary}")
    # Keep only the summary message and the last two messages for context
    new_messages = [new_summary_msg] + state["messages"][-2:]

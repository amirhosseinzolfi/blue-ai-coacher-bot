...
from prompts.prompts import (
    ...
    HELP_TEXT,
    USER_REPORT_PROMPT,
    SUMMARY_PROMPT_TEXT # Assuming you have SUMMARY_PROMPT_TEXT defined, else use SUMMARY_PROMPT if you want to use that instead.
)
...
summary_prompt_text = SUMMARY_PROMPT_TEXT # Or summary_prompt if you prefer
...
############################################
# Type Definitions for AgentState and SummaryAgentState
############################################
from typing import Sequence, Union, Dict, Any, List, Optional, Literal
...
class AgentState(TypedDict):
    messages: Sequence[BaseMessage] # Use BaseMessage to allow SystemMessage, HumanMessage, and AIMessage
    tool_calls: List[ToolCall]
    requires_tool: bool
    current_tool: Optional[str]
    chat_id: str
    username: Optional[str]
    summary: str # Add summary to the state

class SummaryAgentState(AgentState): # Optional: Create a separate state for clarity, inheriting from AgentState
    summary: str
...
def generate_user_report(conversation_text: str, chat_id: str):
    try:
        ...
        from db_manager import save_user_info # Assuming db_manager is in the same directory or PYTHONPATH
...
def should_summarize(state: AgentState) -> Literal["summarize", "continue"]:
    """Determine whether to summarize conversation or continue."""
    messages = state["messages"]
    # Count only HumanMessage and AIMessage for summarization check
    conv_messages = [msg for msg in messages if isinstance(msg, (HumanMessage, AIMessage))]
    if len(conv_messages) > 10: # Summarize if more than 10 messages
        logger.info(f"Conversation length ({len(conv_messages)}) exceeds threshold (10); routing to summarization.")
        return "summarize"
    else:
        return "continue"

def summarize_conversation_node(state: AgentState) -> AgentState:
    """Summarizes the conversation and trims message history."""
    logger.info(f"Entering summarize_conversation_node for chat: {state['chat_id']}")
    conv_messages = [msg for msg in state["messages"] if isinstance(msg, (HumanMessage, AIMessage))]
    if not conv_messages: # No conversation messages to summarize
        logger.info("No conversation messages to summarize.")
        return state

    existing_summary = state.get("summary", "")
    if existing_summary:
        summary_prompt_text_to_use = f"This is summary of the conversation to date: {existing_summary}\n\nExtend the summary by taking into account the new messages above:\n\n"
    else:
        summary_prompt_text_to_use = "Create a summary of the conversation below:\n\n"

    conversation_text = "\n".join([f"{msg.type}: {msg.content}" for msg in conv_messages])
    full_prompt = f"{summary_prompt_text_to_use}{conversation_text}" # Use your SUMMARY_PROMPT_TEXT or SUMMARY_PROMPT
    ...
    try:
        ...
        return state # Return original state if summarization fails

    new_system_message = SystemMessage(content=f"[CONVERSATION SUMMARY]: {new_summary}")
    # Keep only the summary and the last 2 human/AI messages
    last_messages = conv_messages[-2:] if len(conv_messages) > 2 else conv_messages
    new_messages = [new_system_message] + last_messages
    # Ensure to include system messages from before summarization if needed, or just start fresh with summary and last messages.
    # If you have system messages that are important to keep across turns, you may need to adjust message trimming logic.
    all_messages_except_conv = [msg for msg in state["messages"] if not isinstance(msg, (HumanMessage, AIMessage))]
    final_messages = all_messages_except_conv + new_messages # Keep other system messages if any

    logger.info(f"Trimmed conversation: kept {len(final_messages)} messages (summary + last 2 messages + other system messages)")
    logger.process_end("Conversation summarization and trimming completed")

    return {
        "messages": final_messages, # Use the trimmed and summarized messages
        ...
        "summary": new_summary, # Update the summary in the state
    }
...
def agent(state: AgentState):
    ...
    messages = state["messages"]
    summary = state.get("summary", "") # Retrieve summary from state
    ...
    from db_manager import get_user_info # Assuming db_manager is in the same directory or PYTHONPATH
    ...
    try:
        if is_multimodal:
            ...
            if summary: # Include summary as system message if available
                 messages_for_llm.append({"role": "system", "content": f"CONVERSATION SUMMARY: {summary}"})
            for msg in formatted_messages[:-1]:
                messages_for_llm.append(msg)
            messages_for_llm.append({"role": "user", "content": multimodal_content})
            from utils.rich_logger import log_comprehensive_interaction # Assuming utils.rich_logger is in the same directory or PYTHONPATH
            ...
            # Add logging before LLM invocation
        ...
        from utils.helpers import strip_thinking_tags # Assuming utils.helpers is in the same directory or PYTHONPATH
...
def route_to_summary_or_agent(state: AgentState) -> str:
    """Route to summary node if needed, otherwise to agent."""
    if should_summarize(state) == "summarize":
...
workflow = StateGraph(AgentState) # Use AgentState or SummaryAgentState
logger.info("StateGraph initialized with AgentState structure")
workflow.add_node("agent", agent)
workflow.add_node("summarize_conversation", summarize_conversation_node) # Renamed and using new summarize function
logger.info("Added agent and summarize_conversation nodes to workflow")
workflow.add_node("should_summarize", should_summarize) # Add the conditional node
logger.info("Added should_summarize conditional node")

workflow.set_entry_point("agent") # Start at agent node
workflow.add_edge("agent", "should_summarize") # Agent goes to conditional check
workflow.add_conditional_edges(
    "should_summarize",
    {
        "summarize": "summarize_conversation", # If should_summarize returns "summarize"
        "continue": END # If should_summarize returns "continue"
    }
)
workflow.add_edge("summarize_conversation", "agent") # Summarize then go back to agent to continue conversation
logger.add_edge(START, "agent") # Define START edge if you want to start from agent
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    from utils.rich_logger import log_agent_execution # Assuming utils.rich_logger is in the same directory or PYTHONPATH
    ...
    from utils.helpers import refine_ai_response # Assuming utils.helpers is in the same directory or PYTHONPATH
    user_id = str(chat_id)
    ...
    inputs: AgentState = { # Use AgentState or SummaryAgentState here
    ...
    try:
        with MongoDBSaver.from_conn_string(
            ...
            for output in graph.stream(inputs, config=config):
                step_count += 1
                logger.debug(f"Processing step {step_count} of graph execution, output keys: {output.keys()}") # Debug output keys
                ...
                elif "summarize_conversation" in output and "messages" in output["summarize_conversation"]: # Check for summarize_conversation output
                    # Summarization node returns the updated state, agent node will be called again after summarization.
                    logger.debug("Summarization node executed.")
                    pass # No final response from summarization node itself, agent node will generate response after this.
...
def save_message_to_history(chat_id, role, content):
    try:
        ...
        if isinstance(content, list):
            from utils.helpers import format_multimodal_input # Assuming utils.helpers is in the same directory or PYTHONPATH

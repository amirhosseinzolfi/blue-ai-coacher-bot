...
class ConversationState(AgentState):
    ...
    @classmethod
    def from_agent_state(cls, state: AgentState) -> "ConversationState":
        return cls(
            messages=state["messages"],
            tool_calls=state["tool_calls"],
            requires_tool=state["requires_tool"],
...
def optimize_memory(state: AgentState) -> AgentState:
    """Optimized conversation management with smart summarization"""
    SUMMARY_TOKEN_THRESHOLD = 1000  # Adjust based on your needs
    
    # Convert to ConversationState if needed
    conv_state = (
        state if isinstance(state, ConversationState) 
        else ConversationState.from_agent_state(state)
    )
    
    # Get messages since last summary
    if conv_state.summary:
        summary_idx = next(
            (i for i, msg in enumerate(conv_state["messages"]) 
             if isinstance(msg, SystemMessage) and msg.content.startswith("[CONVERSATION SUMMARY]:")),
            0
        )
        new_messages = conv_state["messages"][summary_idx + 1:]
    else:
        new_messages = conv_state["messages"]
    
    # Check if summarization is needed
    if not should_summarize(new_messages, SUMMARY_TOKEN_THRESHOLD):
        return state
        
    # Generate new summary
    summary_text = generate_summary(new_messages, conv_state.summary)
    
    # Create new state with summary
    new_system = SystemMessage(content=f"[CONVERSATION SUMMARY]: {summary_text}")
    # Keep last 2 messages for immediate context
    recent_messages = new_messages[-2:] if len(new_messages) >= 2 else new_messages
    conv_state["messages"] = [new_system] + recent_messages
    conv_state.summary = summary_text
    conv_state.last_summary_time = time.time()
    
    return conv_state

def should_summarize(messages: List[Union[SystemMessage, HumanMessage, AIMessage]], threshold: int) -> bool:
    ...
    total_tokens = sum(
        ...
        for msg in messages
        if not (isinstance(msg, SystemMessage) and msg.content.startswith("[CONVERSATION SUMMARY]:"))
    )
    
    return total_tokens > threshold

def generate_summary(new_messages: List[Union[SystemMessage, HumanMessage, AIMessage]], 
                    previous_summary: str) -> str:
    """Generate an optimized summary of the conversation"""
    conversation_text = "\n".join([

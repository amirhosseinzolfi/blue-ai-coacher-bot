...
def new_chat_session(chat_id: str) -> str:
    ...
    Creates a new chat session for the given chat_id:
      ...
      - Leaves previous session summaries and chat messages intact.
    Returns the new session ID.
    """
    new_session_id = f"{chat_id}_{int(datetime.datetime.now().timestamp())}"
    
    # Save the new session ID using the existing helper function.
    save_session_id(chat_id, new_session_id)
    
    logger.info("New chat session '%s' started for chat '%s'. Previous session history is preserved.", new_session_id, chat_id)

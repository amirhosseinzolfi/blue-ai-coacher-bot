...
def optimize_memory(state: AgentState) -> AgentState:
    ...
    db_manager.save_conversation_summary(state["chat_id"], get_session_id(state["chat_id"]), new_summary)
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    from db_manager import db_manager
    conversation_summary = db_manager.get_conversation_summary(user_id, session_id)
...
def save_message_to_history(chat_id: str, role: str, content: str, session_id: Optional[str] = None) -> None:
    ...
    If session_id is provided, it will be used; otherwise, a default is constructed.
    """
    from db_manager import save_message_to_history as db_save
    db_save(chat_id, role, content, session_id or f"{chat_id}_default")
...
def new_chat_session(chat_id: str) -> str:
    """
    Creates a new chat session for the given chat_id by generating a new unique session ID.
    The previous session data remains stored.
    """
    ...
    logger.info("New chat session '%s' started for chat '%s'. Previous session history and business info are preserved.", new_session_id, chat_id)

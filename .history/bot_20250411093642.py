...
def new_chat(message):
    ...
    This command resets user report and session history.
    """
    ...
    new_session_id = f"{chat_id_str}_{int(datetime.datetime.now().timestamp())}"

    # Import necessary functions
    from telegram_bot import save_session_id # This function now handles int/str conversion
    # Import save_business_info directly from db_manager to clear the report
    from db_manager import save_business_info

    # Clear user report by saving an empty string under the 'user_report' key
    # Note: This assumes save_business_info merges data. If it overwrites, this is fine.
    # If it requires loading existing data first, this approach needs adjustment.
    # Based on db_manager.py, update_one with $set will only update specified fields.
    save_business_info(int_chat_id, {"user_report": ""})

    # Save the new session ID (function handles conversion)
    save_session_id(int_chat_id, new_session_id)

    logger.info("New chat session created. Chat: %s, Session: %s", chat_id_str, new_session_id)

    response_text = "🆕 جلسه چت جدید ایجاد شد. جلسه قبلی در تاریخچه ذخیره شده است و تاریخچه چت و گزارش کاربر بازنشانی شد."
    ...
    save_message_to_history(chat_id_str, "system", welcome_text)
...
def handle_daily_tasks(call):
    ...
    logger.info("Daily Tasks button clicked by %s in chat %s", sender_first_name, chat_id)
    ...
    try:
        ...
        logger.info("Daily task response sent to %s", sender_first_name)
        ...
        logger.error("Error in Daily Tasks handler: %s", str(e))
...
def handle_instagram_story_idea(call):
    chat_id = str(call.message.chat.id)
    sender_first_name = call.from_user.first_name or call.from_user.username
    logger.info("Instagram Story idea button clicked by %s in chat %s", sender_first_name, chat_id)
    ...
    try:
        refined_response = run_agent(f"{sender_first_name} : {prompt_input}", chat_id, placeholder_message.message_id, sender_first_name)
        save_message_to_history(chat_id, "assistant", refined_response)
        logger.info("Instagram story idea sent to %s", sender_first_name)
        bot.edit_message_text(refined_response, chat_id=chat_id,
                              message_id=placeholder_message.message_id,
                              parse_mode="MarkdownV2")
    except Exception as e:
...
def handle_chat_report(call):
    chat_id = str(call.message.chat.id)
    sender_first_name = call.from_user.first_name or call.from_user.username
    logger.info("Chat Report button clicked by %s in chat %s", sender_first_name, chat_id)
    ...
    try:
        refined_response = run_agent(f"{sender_first_name} : {prompt_input}", chat_id, placeholder_message.message_id, sender_first_name)
        save_message_to_history(chat_id, "assistant", refined_response)
        logger.info("Chat report sent to %s", sender_first_name)
        bot.edit_message_text(refined_response, chat_id=chat_id,
                              message_id=placeholder_message.message_id,
                              parse_mode="MarkdownV2")
    except Exception as e:
...
def handle_shamsi_date(call):
    chat_id = str(call.message.chat.id)
    sender_first_name = call.from_user.first_name or call.from_user.username
    logger.info("Shamsi Date button clicked by %s in chat %s", sender_first_name, chat_id)
    ...
    try:
        ...
        refined_response = run_agent(f"{sender_first_name} : {prompt_input}", chat_id, placeholder_message.message_id, sender_first_name)
        save_message_to_history(chat_id, "assistant", refined_response)
        logger.info("Shamsi date response sent to %s", sender_first_name)
        bot.edit_message_text(refined_response, chat_id=chat_id,
                              message_id=placeholder_message.message_id,
                              parse_mode="MarkdownV2")
    except Exception as e:
        error_message = escape_markdown_v2(f"❌ متأسفم، مشکلی در پردازش تاریخ شمسی پیش آمد. تاریخ شمسی امروز: {shamsi_date}")
        logger.error("Error in Shamsi Date handler: %s", str(e))
        bot.edit_message_text(error_message, chat_id=chat_id,
                              message_id=placeholder_message.message_id,
                              parse_mode="MarkdownV2")
...
def handle_message(message):
    ...
    if message.content_type == 'photo':
        ...
        logger.info("Photo received from %s in chat %s with caption: %s", sender_first_name, chat_id, text_component)
    else:
        query_payload = message.text
        user_message_text = message.text

    logger.info("Received message from %s in chat %s", sender_first_name, chat_id)
    ...
    if chat_type in ['group', 'supergroup']:
        ...
        if not is_mentioned:
            logger.debug("Message in group %s not mentioning bot - no response", chat_id)
            return

    bot.send_chat_action(chat_id, 'typing')
    placeholder_message = bot.reply_to(message, escape_markdown_v2("🤔 در حال پردازش..."), parse_mode="MarkdownV2")
    ...
    try:
        ...
        bot.edit_message_text(refined_response, chat_id=chat_id,
                              message_id=placeholder_message.message_id,
                              parse_mode="MarkdownV2")
        logger.info("Response sent to %s", sender_first_name)
        save_message_to_history(chat_id, "assistant", refined_response)
        
    except Exception as e:
        logger.error("Error processing message for chat %s: %s", chat_id, e, exc_info=True)
        error_message = escape_markdown_v2("❌ متأسفم، مشکلی در پردازش درخواست شما پیش آمد. لطفاً دوباره تلاش کنید.")
        bot.edit_message_text(error_message, chat_id=chat_id,
                              message_id=placeholder_message.message_id,
                              parse_mode="MarkdownV2")
...
############################################
# Main Bot Execution and Cleanup
############################################
def main():
    logger.info("Setting up bot commands...")
    setup_bot_commands()
    logger.info("Telegram Bot is now running. Group conversations are logged and used as context.")
    bot.polling(none_stop=True)

def cleanup_sessions():
    """
    Ensures that all aiohttp sessions are properly closed.
    """
    try:
        if hasattr(client, 'session') and client.session:
            client.session.close()
            logger.info("Closed g4f aiohttp session.")
    except Exception as e:

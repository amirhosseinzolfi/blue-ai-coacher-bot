...
def optimize_memory(state: AgentState) -> AgentState:
    ...
    logger.process_start(f"[bold yellow]🔄 Starting conversation summarization for chat {chat_id}[/bold yellow]")
    ...
    human_messages = [msg for msg in conv_messages if isinstance(msg, HumanMessage)]
    ai_messages = [msg for msg in conv_messages if isinstance(msg, AIMessage)]
    
    table = Table(title=f"[bold blue]Conversation Summary Stats - Chat {chat_id}[/bold blue]")
    table.add_column("Metric", style="cyan")
    table.add_column("Count", style="green")
    table.add_row("Total Messages", str(len(conv_messages)))
    table.add_row("User Messages", str(len(human_messages)))
    table.add_row("AI Messages", str(len(ai_messages)))
    console.print(table)
    ...
    try:
        ...
        # Display old vs new summary comparison
        comparison_table = Table(title="[bold magenta]Summary Comparison[/bold magenta]")
        comparison_table.add_column("Type", style="cyan")
        comparison_table.add_column("Content", style="yellow", width=50)
        if existing_summary:
            comparison_table.add_row("Previous Summary", existing_summary[:200] + "..." if len(existing_summary) > 200 else existing_summary)
        comparison_table.add_row("New Summary", new_summary[:200] + "..." if len(new_summary) > 200 else new_summary)
        console.print(comparison_table)
        
        logger.info(f"[green]✅ Summarization completed in {duration:.2f}s[/green]")
        logger.info(f"[cyan]Original message count: {len(conv_messages)} → New count: {len(conv_messages[-2:]) + 1}[/cyan]")
    ...
    logger.info(f"[blue]Conversation trimmed to {len(new_messages)} messages (summary + last 2)[/blue]")
...
class MessageCounter:
    ...
    def increment_and_check(self, chat_id: str, is_user_message: bool = False) -> bool:
        ...
        with self._lock:
            ...
            # Enhanced logging with progress bar visual
            progress = "🟦" * count + "⬜" * (10 - count)
            logger.info(f"[cyan]Message counter for chat {chat_id}: [{progress}] ({count}/10)[/cyan]")
            
            if count >= 10:
                logger.info(f"[yellow]🔄 Triggering summarization after {count} user messages[/yellow]")

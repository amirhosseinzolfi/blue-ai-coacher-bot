    ...
    try:
        while True:
            ...
            try:
                ...
                # Assemble structured output via a rich Table
                resp_table = Table(show_header=False, box=None, expand=True)
                resp_table.add_row("[bold green]Content:[/bold green]", content)
                resp_table.add_row("[bold blue]Additional Args:[/bold blue]", str(additional_kwargs))
                resp_table.add_row("[bold magenta]Resp. Metadata:[/bold magenta]", str(response_metadata))
                resp_table.add_row("[bold cyan]Usage Metadata:[/bold cyan]", str(usage_metadata))

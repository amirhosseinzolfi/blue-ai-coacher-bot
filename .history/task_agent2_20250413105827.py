...
def format_llm_response(response):
    """Format LLM response into structured sections"""
    try:
        ...
        usage_table = Table(show_header=True, header_style="bold cyan", box=None)
        usage_table.add_column("Metric", style="blue")
        usage_table.add_column("Value", style="green")
        
        if 'token_usage' in metadata:
            for k, v in metadata['token_usage'].items():
                if not isinstance(v, dict):  # Only show simple metrics
                    usage_table.add_row(k, str(v))

        # Format response sections
        sections = [
            Panel(content, title="[bold cyan]Response Content[/bold cyan]", border_style="cyan"),
            Panel(usage_table, title="[bold blue]Token Usage[/bold blue]", border_style="blue")
        ]
        ...
        return Panel(str(response), title="[bold yellow]Raw Response[/bold yellow]", border_style="yellow")
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            # Process LLM with enhanced logging
            ...
            try:
                ...
                console.print(Panel(
                    f"[bold red]Error:[/bold red] {llm_err}",

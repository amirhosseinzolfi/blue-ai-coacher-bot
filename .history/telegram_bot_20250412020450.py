...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Enhanced conversation summarization with better triggers and retention.
    """
    SUMMARY_THRESHOLD = 8  # Messages before summarization
    RETAIN_MESSAGES = 3    # Recent messages to keep
    ...
    # Extract previous summary if exists
    previous_summary = ""
    if conv_messages and isinstance(conv_messages[0], SystemMessage):
        if conv_messages[0].content.startswith("[SUMMARY]:"):
            previous_summary = conv_messages[0].content.replace("[SUMMARY]:", "").strip()
    ...
    # Create conversation text focusing on key interactions
    conversation_text = "\n".join([
    ...
    try:
        ...
        new_system = SystemMessage(content=f"[SUMMARY]: {new_summary}")
        retained_messages = new_messages_for_summary[-RETAIN_MESSAGES:]
        new_messages = [new_system] + retained_messages
        
        logger.info(f"Summarized conversation in {duration:.2f}s. Kept {len(retained_messages)} recent messages.")
...
class MessageCounter:
    def __init__(self):
        ...
        self.MIN_TIME_BETWEEN_SUMMARIES = 300  # 5 minutes

    def should_summarize(self, chat_id: str) -> bool:
        with self._lock:
            current_time = time.time()
            last_time = self._last_summary_time.get(chat_id, 0)
            message_count = self._counters.get(chat_id, 0)
            
            if (current_time - last_time >= self.MIN_TIME_BETWEEN_SUMMARIES and 
                message_count >= 8):
                self._counters[chat_id] = 0
                self._last_summary_time[chat_id] = current_time
                return True
            
            self._counters[chat_id] = message_count + 1
            return False
...
def agent(state: AgentState):
    ...
    try:
        ...
        if message_counter.should_summarize(chat_id):

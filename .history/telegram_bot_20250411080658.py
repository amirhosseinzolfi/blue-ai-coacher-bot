...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Now just returns state immediately and triggers background summarization
    """
    chat_id = state['chat_id']
    if message_counter.increment_and_check(chat_id):
        background_optimize_memory(state)
    return state

@run_in_background
def summarize_conversation(messages, existing_summary, chat_id):
    """
    ...
    try:
        ...
        summary_prompt_text = (f"This is the summary so far: {existing_summary}\n"
                             ...
                             if existing_summary else "Create a summary of the conversation above:")
...
def background_optimize_memory(state: AgentState):
    """
    Triggers background summarization without blocking
    """
    chat_id = state['chat_id']
    conv_messages = [msg for msg in state["messages"] if isinstance(msg, (HumanMessage, AIMessage))]
    existing_summary = state.get("summary", "")
    
    summarize_conversation(conv_messages, existing_summary, chat_id)
    return state

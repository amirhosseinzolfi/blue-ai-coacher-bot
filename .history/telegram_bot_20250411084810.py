...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Summarize conversation keeping only system messages and last 2 messages.
    """
    ...
    try:
        summary_response = llm_summary.invoke(messages_for_summary)
        new_summary = summary_response.content.strip()
        
        # Keep system messages + last 2 conversation messages
        ...
        log_summarization(logger, "\n".join(str(m) for m in messages), new_summary, "Conversation")
...
def should_continue(state: AgentState) -> Literal["optimize_memory", END]:
    """Determine whether to summarize conversation or end."""
    messages = state["messages"]
    # If there are more than 10 messages, summarize the conversation
    if len([m for m in messages if isinstance(m, (HumanMessage, AIMessage))]) > 10:

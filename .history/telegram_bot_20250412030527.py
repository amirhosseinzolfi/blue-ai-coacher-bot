...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Create structured, LLM-optimized summaries of conversations with explicit context preservation.
    Inject the full conversation history as system instruction to the LLM for improved context retention.
    """
    THRESHOLD = 6  # lowered threshold for more efficient summarization
    ...
    # Check for an existing summary and prepare structured instruction
    if conv_messages and isinstance(conv_messages[0], SystemMessage) and conv_messages[0].content.startswith("[CONVERSATION SUMMARY]:"):
        ...
        summary_instruction = (
            ...
            "<<TASK>>\n"
            "Extend the conversation summary while preserving key insights, business context, and pending items.\n"
        )
        ...
        summary_instruction = (
            "<<NEW CONVERSATION>>\n"
            "Create a structured summary with key insights, business context, and pending items.\n"
        )
        new_messages_for_summary = conv_messages

    # Build conversation history text with timestamps
    conversation_text = "\n".join([
        f"[{msg.__class__.__name__}]({int(time.time())}): {msg.content}"
        for msg in new_messages_for_summary
    ])
    
    # Include the entire conversation history as a system instruction
    full_prompt = (
        f"{SUMMARY_PROMPT_TEXT}\n\n"
        "<<SYSTEM INSTRUCTION>>\n"
        "You are a professional business assistant. Use the following conversation history to provide full context:\n"

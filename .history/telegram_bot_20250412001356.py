...
def agent(state: AgentState):
    ...
    # New: Load permanent context from MongoDB (replace in‑memory maps)
    from db_manager import db_manager
    context = db_manager.load_chat_context(chat_id, chat_type="private")
    ai_tone = context["settings"].get("ai_tone", "دوستانه")
    business_info = context["business_info"]
    # Use a default message if business info is empty
    if not business_info:
        business_info = "No business information provided."
    logger.info(f"BUSINESS INFO loaded: {business_info}")

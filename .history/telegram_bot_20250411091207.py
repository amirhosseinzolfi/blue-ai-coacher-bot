...
# Define MessagesState base class
...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Summarize conversation after 10 messages, keeping the last 2 messages.
    """
    logger.info(f"Checking message count for chat: {state['chat_id']}")
    conv_messages = [msg for msg in state["messages"] if isinstance(msg, (HumanMessage, AIMessage))]
    if len(conv_messages) <= 10:
        logger.debug(f"Message count ({len(conv_messages)}) below threshold (10), skipping summarization")
        return state
    ...
    if existing_summary:
        summary_prompt = (
            f"This is the summary of the conversation to date: {existing_summary}\n\n"
            "Extend the summary by incorporating the new messages above:"
        )
    else:
        summary_prompt = "Create a professional summary of the conversation above:"

    try:
        conversation_text = "\n".join([
            f"[{msg.type}]: {msg.content}" for msg in conv_messages
        ])
        messages = conv_messages + [HumanMessage(content=summary_prompt)]
        
        start_time = time.time()
        summary_response = llm_summary.invoke(messages)
        duration = time.time() - start_time
        
        new_summary = summary_response.content.strip()
        logger.info(f"Generated new summary in {duration:.2f}s")
        
        # Keep system message (if any) + last 2 messages
        system_messages = [msg for msg in state["messages"] if isinstance(msg, SystemMessage)]
        new_messages = system_messages + conv_messages[-2:]
        
        logger.info(f"Trimmed conversation: kept {len(new_messages)} messages (including {len(system_messages)} system messages)")
...
def background_optimize_memory(state: AgentState):
    """Runs memory optimization in background after 10 messages."""
    chat_id = state['chat_id']
    ...
    def run_summarization():
        try:
            ...
            # Save summary to MongoDB using db_manager
            from db_manager import db_manager
            db_manager.save_conversation_summary(chat_id, new_summary)
        except Exception as e:
...
def agent(state: AgentState):
    ...
    try:
        ...
        # Trigger background memory optimization *before* returning the new state
...
def should_continue(state: State) -> Literal["summarize_conversation", END]:
    ...
    messages = state["messages"]
    # If there are more than 10 messages, then we summarize the conversation
    if len(messages) > 10:
        return "summarize_conversation"
    return END

def summarize_conversation(state: State):
    """Summarize conversation and keep last 2 messages."""
    ...
    try:
        with MongoDBSaver.from_conn_string(
            ...
            for output in graph.stream(inputs, config=config):
                ...
                if "summarize_conversation" in output:
                    # Handle summarization results
                    logger.info("Conversation summarized, updating state...")
                    if "summary" in output["summarize_conversation"]:
                        logger.info(f"New summary generated: {output['summarize_conversation']['summary'][:100]}...")
                
                if "conversation" in output:
                    most_recent = output["conversation"]["messages"][-1]
                    if isinstance(most_recent, AIMessage):
                        final_response = most_recent.content
                        
            if not final_response:
                final_response = "متأسفم، نتوانستم پاسخ مناسبی بیابم. لطفا دوباره تلاش کنید."
            
            return refine_ai_response(final_response.strip())

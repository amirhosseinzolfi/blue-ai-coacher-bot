...
# Create a background task executor and queue
background_executor = ThreadPoolExecutor(max_workers=4)
background_tasks = queue.Queue()

def run_in_background(fn):
    """Decorator to run a function in background"""
    @functools.wraps(fn)
...
@run_in_background
def save_message_history_async(chat_id, role, content):
    """Asynchronous version of save_message_to_history"""
    try:
        save_message_to_history(chat_id, role, content)
    except Exception as e:
...
def handle_message(message):
    ...
    # Quick early returns for pending updates
    if ai_tone_update_pending.get(chat_id) or business_info_update_pending.get(chat_id):
        # ...existing code for handling pending updates...
        return
    ...
    save_message_history_async(chat_id, "user", f"{sender_first_name}: {user_message_text}")
    ...
    try:
        # Get AI response as quickly as possible
        ...
        save_message_history_async(chat_id, "assistant", refined_response)
        
        # Log success in background
        background_executor.submit(logger.info, f"[bold green]✅ Response sent to {sender_first_name}[/bold green]")
        ...
        bot.edit_message_text(error_message, chat_id=chat_id,
                          message_id=placeholder_message.message_id,
                          parse_mode="MarkdownV2")
        logger.error(f"[bold red]❌ Error processing message: {str(e)}[/bold red]")

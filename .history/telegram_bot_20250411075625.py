...
def run_agent(query, chat_id, message_id, username=None):
    ...
    logger.debug("Created HumanMessage with formatted prompt")
    ...
    try:
        with MongoDBSaver.from_conn_string(
            ...
            final_response = None
            step_count = 0
            # Stream the workflow and break once the agent node produces a response.
            for output in graph.stream(inputs, config=config):
                ...
                if "agent" in output and "messages" in output["agent"]:
                    ...
                    if isinstance(most_recent_msg, AIMessage):
                        ...
                        logger.debug("Got response from agent node; breaking stream loop")
                        break
            # Launch background summarization (if needed) without delaying the main response.
            import threading
            threading.Thread(target=background_optimize_memory, args=(inputs.copy(),), daemon=True).start()

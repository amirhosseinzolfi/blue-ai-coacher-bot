...
def new_chat_session(chat_id: str) -> str:
    """
    Resets the chat session for the given chat_id:
      - Generates a new session ID.
      - Clears previous chat history from the chat_history collection.
      - Clears the conversation summary.
      - Saves the new session ID in the business_info collection.
    ...
    collection = get_mongo_collection()
    deletion_result = collection.delete_many({"session_id": {"$regex": f"^{chat_id}_"}})
    logger.info("Cleared %d messages for chat '%s' (old sessions).", deletion_result.deleted_count, chat_id)
    
    # Also clear conversation summary via db_manager.
    from db_manager import db_manager
    db_manager.save_conversation_summary(chat_id, "")
    
    logger.info("New chat session '%s' started for chat '%s'.", new_session_id, chat_id)

...
from utils.rich_logger import (
    ...
    format_conversation_summary_table, display_conversation_preview  # Add these imports
)
...
def optimize_memory(state: AgentState) -> AgentState:
    ...
    # Show message preview before summarization
    try:
        preview = display_conversation_preview(conv_messages)
        console.print(preview)
        logger.info("[cyan]Message preview displayed[/cyan]")
    except Exception as e:
        logger.error(f"[red]Error displaying message preview: {e}[/red]")
    ...
    try:
        ...
        try:
            summary_table = format_conversation_summary_table(conv_messages, new_summary)
            console.print(summary_table)
            logger.info("[cyan]Summary statistics displayed[/cyan]")
        except Exception as e:
            logger.error(f"[red]Error displaying summary statistics: {e}[/red]")
        
        logger.info(f"[bold green]✨ Summary generated in {duration:.2f}s[/bold green]")
    ...
    logger.info(f"[blue]Kept {len(new_messages)} messages (1 summary + {len(conv_messages[-2:])} recent)[/blue]")

...
def handle_message(message):
    ...
    logger.info("-" * 40)
    logger.info(f"Message from {sender_first_name} in {chat_type} {chat_id}")
    
    if message.content_type == 'photo':
        text_component = message.caption if message.caption else ""
        logger.info(f"📷 Photo received with caption: {text_component[:50]}...")
    else:
    ...
    try:
        ...
        logger.info(f"✅ Response sent ({len(refined_response)} chars)")
        bot.edit_message_text(refined_response, chat_id=chat_id,
                              message_id=placeholder_message.message_id,
                              parse_mode="MarkdownV2")

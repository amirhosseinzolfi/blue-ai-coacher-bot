...
def optimize_memory(state: AgentState) -> AgentState:
    # New thresholds for triggering summarization
    THRESHOLD_COUNT = 10
    THRESHOLD_LENGTH = 1000  # total characters threshold
    conv_messages = [msg for msg in state["messages"] if isinstance(msg, (HumanMessage, AIMessage, SystemMessage))]
    total_length = sum(len(msg.content) for msg in conv_messages if hasattr(msg, 'content'))
    if len(conv_messages) < THRESHOLD_COUNT and total_length < THRESHOLD_LENGTH:
        logger.info(f"Conversation size within limits: messages={len(conv_messages)}, length={total_length} (thresholds: {THRESHOLD_COUNT} msgs, {THRESHOLD_LENGTH} chars)")
        return state

    # If a previous summary exists, exclude it from the new summary input
    ...
    # Prune messages: keep the new summary and the last 3 messages
    remaining_messages = new_messages_for_summary[-3:] if len(new_messages_for_summary) >= 3 else new_messages_for_summary
    new_messages = [new_system] + remaining_messages
    logger.info(f"Trimmed conversation: kept {len(new_messages)} messages (summary + last 3 messages)")

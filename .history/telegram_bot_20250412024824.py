...
def prepare_system_context(summary: str, state: AgentState) -> str:
    """Prepare conversation context as system instructions"""
    return (
        "<<CONVERSATION CONTEXT>>\n"
        f"• Previous Interactions: {summary}\n"
        f"• Active User: {state.get('username', 'Unknown')}\n"
        f"• Session ID: {state['chat_id']}\n"
        "<<IMPORTANT POINTS>>\n"
        "• Reference this history when responding\n"
        "• Maintain context continuity\n"
        "• Keep track of mentioned tasks and commitments\n"
        "<<END CONTEXT>>"
    )

def optimize_memory(state: AgentState) -> AgentState:
    """Create structured, LLM-optimized summaries with system context integration"""
    THRESHOLD = 6
    ...
    if conv_messages and isinstance(conv_messages[0], SystemMessage) and conv_messages[0].content.startswith("[CONVERSATION SUMMARY]:"):
        ...
        summary_instruction = (
            "<<SYSTEM INSTRUCTION>>\n"
            "Summarize the following conversation while maintaining these key elements:\n"
            "1. Ongoing tasks and commitments\n"
            "2. User preferences and business context\n"
            "3. Important decisions and action items\n"
            "4. Timeline of key events\n\n"
            "<<PREVIOUS CONTEXT>>\n"
            f"{previous_summary}\n"
            "<<CURRENT CONTEXT>>\n"
            f"User: {state.get('username', 'Unknown')}\n"
            f"Session: {state['chat_id']}\n"
        )
        ...
        summary_instruction = (
            "<<SYSTEM INSTRUCTION>>\n"
            "Create a comprehensive summary capturing:\n"
    ...
    # Update system context with the new summary
    system_context = prepare_system_context(new_summary, state)
    
    # Create new system message with both summary and context
    ...
    # Combine system context with remaining messages

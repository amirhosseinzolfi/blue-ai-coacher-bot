...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Summarize conversation if there are more than a threshold number of messages.
    Retain the summary and last N messages for context.
    """
    THRESHOLD = 6       # reduced threshold for earlier summarization
    MAX_RECENT = 2      # number of recent messages to retain
    ...
    # Exclude any existing summary messages
    new_messages_for_summary = [msg for msg in conv_messages if not (isinstance(msg, SystemMessage) and msg.content.startswith("[CONVERSATION SUMMARY]:"))]
    ...
    remaining_messages = new_messages_for_summary[-MAX_RECENT:] if len(new_messages_for_summary) >= MAX_RECENT else new_messages_for_summary
    new_messages = [summary_message] + remaining_messages
    logger.info(f"Trimmed conversation: kept {len(new_messages)} messages (summary + last {MAX_RECENT} messages)")

...
def new_chat(message):
    """
    /new_chat Command: Archive previous session’s history and summary,
    then create a new chat session. The previous session’s messages and
    summary are saved permanently.
    """
    ...
    from telegram_bot import new_chat_session, save_message_to_history, llm_summary
    from db_manager import get_session_id, get_chat_history, save_conversation_summary

    # Archive previous session:
    current_session = get_session_id(chat_id_str)
    if current_session:
        history = get_chat_history(chat_id_str, session_id=current_session)
        if history:
            # Combine the content of all messages.
            combined_text = "\n".join([msg.get("content", "") for msg in history])
            # Build a prompt for summarization.
            summary_prompt = f"لطفاً خلاصه‌ای ساختاریافته از گفتگو زیر به زبان فارسی ارائه بده:\n{combined_text}\n\n• خلاصه کلیدی:\n• زمینه کسب و کار:\n• موارد در حال پیگیری:"
            from langchain.schema import HumanMessage
            response = llm_summary.invoke([HumanMessage(content=summary_prompt)])
            summary_text = response.content.strip()
            # Save the summary for the current session.
            save_conversation_summary(chat_id_str, current_session, summary_text)
            logger.info(f"Archived summary for session '{current_session}' of chat '{chat_id_str}'.")
    ...
    response_text = "🆕 جلسه چت جدید ایجاد شد. تاریخچه قبلی بایگانی و خلاصه‌سازی شده است؛ جلسه جدید آغاز گردید."
    ...
    # Now pass the new_session_id to store the welcome message with the new session.
    save_message_to_history(chat_id_str, "system", welcome_text, new_session_id)

...
**your task** :
- organize and handle users(workers) tasks and requests based on chat history and tasks which user told you (remeber dont set tasks by your self if user dont tell you that or if there isnt any task set by user in history) , dont create any sample or unreal tasks and info ata all
...
SUMMARY_PROMPT_TEXT = """لطفاً مکالمه زیر را با تمرکز بر نکات کلیدی کسب‌وکار و وظایف کاربران به صورت خلاصه و مفید خلاصه کن.
در صورتی که خلاصه قبلی موجود است، آن را در نظر بگیر و خلاصه جدید را به‌روز کن.
خلاصه قبلی:
{previous_summary}

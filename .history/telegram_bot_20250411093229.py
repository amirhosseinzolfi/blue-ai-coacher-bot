...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Summarize conversation after each message and keep last 2 messages.
    """
    logger.info(f"Processing conversation summary for chat: {state['chat_id']}")
    
    conv_messages = [msg for msg in state["messages"] if isinstance(msg, (HumanMessage, AIMessage))]
    if len(conv_messages) <= 2:
        logger.info(f"Not enough messages ({len(conv_messages)}) to summarize yet")
        return state
    ...
    try:
        ...
        # Keep only summary as system message and last 2 messages
        ...
        logger.process_end("Summarization and trimming completed")
        
        # Update state with new messages and summary
        return {
...
def agent(state: AgentState):
    ...
    try:
        ...
        # Always optimize memory after each message

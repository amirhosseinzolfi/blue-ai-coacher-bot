...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Summarize conversation to maintain context, focusing on key details
    and structured formatting for AI comprehension.
    """
    THRESHOLD = 6
    conv_messages = [msg for msg in state["messages"] if isinstance(msg, (HumanMessage, AIMessage, SystemMessage))]
    ...
    # Extract existing summary and prepare structured format
    if conv_messages and isinstance(conv_messages[0], SystemMessage) and conv_messages[0].content.startswith("[CONVERSATION SUMMARY]:"):
        previous_summary = conv_messages[0].content.replace("[CONVERSATION SUMMARY]:", "").strip()
        summary_instruction = (
            "<<CONTEXT>>\n"
            f"Previous Summary: {previous_summary}\n"
            "<<USER INFO>>\n"
            f"Username: {state.get('username', 'Unknown')}\n"
            f"Chat ID: {state['chat_id']}\n"
            "<<TASK>>\n"
            "Extend the conversation summary while preserving:\n"
            "1. Key business decisions and tasks\n"
            "2. User preferences and important details\n"
            "3. Ongoing conversations and pending items\n"
            "Format as: [KEY POINTS], [BUSINESS CONTEXT], [PENDING ITEMS]\n"
        )
        new_messages_for_summary = conv_messages[1:]
    else:
        previous_summary = ""
        summary_instruction = (
            "<<NEW CONVERSATION>>\n"
            ...
            "[PENDING ITEMS]: Ongoing tasks or unanswered questions\n"
        )
        new_messages_for_summary = conv_messages

    # Create structured conversation text with timestamps
    conversation_text = "\n".join([
        f"[{msg.__class__.__name__}]({int(time.time())}): {msg.content}"
        for msg in new_messages_for_summary
    ])
    
    # Build enhanced prompt for better LLM comprehension
    full_prompt = (
        ...
        f"{conversation_text}\n\n"
        "<<FORMAT>>\n"
        "Return summary in Persian with clear sections:\n"
        "• خلاصه کلیدی:\n"
        "• زمینه کسب و کار:\n"
        "• موارد در حال پیگیری:\n"
    )
    ...
    # Format the final summary with explicit markers for LLM
    new_system = SystemMessage(content=(
        "[CONVERSATION SUMMARY]:\n"
        "<<SUMMARY_START>>\n"
        f"{new_summary}\n"
        "<<SUMMARY_END>>\n"
        f"<<LAST_UPDATE>>{int(time.time())}>>\n"
    ))

    # Save the summary into the database for future loads
    from db_manager import db_manager
    db_manager.save_conversation_summary(state["chat_id"], new_summary)

    # Keep only the summary and the last two messages of the conversation
    remaining_messages = new_messages_for_summary[-2:] if len(new_messages_for_summary) >= 2 else new_messages_for_summary
    new_messages = [new_system] + remaining_messages
...
def agent(state: AgentState):
    ...
    try:
        ...
        # Run summarization only every 5 messages (user + AI)
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    # Load the consolidated conversation summary from the database and prepend it
    from db_manager import db_manager
    conversation_summary = db_manager.get_conversation_summary(user_id)
    if conversation_summary:
        from langchain.schema import SystemMessage
        summary_msg = SystemMessage(content=f"[CONVERSATION SUMMARY]:\n{conversation_summary}")
        history_messages = [summary_msg] + history_messages
...
def get_mongo_collection():
    # Use the persistent client from db_manager
    from db_manager import db_manager
    return db_manager.db[COLLECTION_NAME]

def get_business_info_collection():
    from db_manager import db_manager
    return db_manager.db[BUSINESS_INFO_COLLECTION]
...
def get_user_info(chat_id, date=None):
    ...
    collection = get_mongo_collection()  # assuming user_info is stored in the main collection if not, adjust accordingly
    doc = collection.find_one({"chat_id": chat_id, "date": today})
...
def save_message_to_history(chat_id: str, role: str, content: str, session_id: Optional[str] = None) -> None:
    """
    Save a message to the chat history collection with an optional session ID.
    """
    try:
        from langchain_mongodb.chat_message_histories import MongoDBChatMessageHistory
        history_obj = get_history_for_chat(chat_id, session_id)
        if role == "user":
            from langchain.schema import HumanMessage
        ...
        logger.info("Saved message to history for chat '%s'. Role: '%s', Content: '%s', Session ID: '%s'", chat_id, role, truncated, session_id)
    except Exception as e:
...
def new_chat_session(chat_id: str) -> str:
    """
    Creates a new chat session for the given chat_id:
      - Generates a new unique session ID.
      - Updates the session ID in the business_info collection so that new messages are stored under this new session.
      - Leaves previous session summaries and business info intact.
    ...
    # Save the new session ID in the database
    db_manager.save_setting(chat_id, "current_session_id", new_session_id)

    logger.info(f"New chat session '{new_session_id}' started for chat '{chat_id}'. Previous session history is preserved.")

...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            # Process LLM with structured logging of response components
            ...
            try:
                ...
                if isinstance(response, dict):
                    ...
                    resp_meta       = response.get('response_metadata', {})
                    usage_meta      = response.get('usage_metadata', {})
                    reasoning_tokens = resp_meta.get('completion_tokens_details', {}).get('reasoning_tokens', 'N/A')
                else:
                    content         = str(response)
                    model = finish = resp_id = system_fp = ""
                    additional = resp_meta = usage_meta = {}
                ...
                resp_table.add_row("[bold blue]Model:[/bold blue]", model)
                resp_table.add_row("[bold magenta]Finish Reason:[/bold magenta]", finish)
                resp_table.add_row("[bold cyan]ID:[/bold cyan]", resp_id)
                resp_table.add_row("[bold yellow]System FP:[/bold yellow]", system_fp)
                resp_table.add_row("[bold red]Reasoning Tokens:[/bold red]", str(reasoning_tokens))
                resp_table.add_row("[bold yellow]Additional Args:[/bold yellow]", str(additional))
                resp_table.add_row("[bold yellow]Response Metadata:[/bold yellow]", str(resp_meta))
                resp_table.add_row("[bold yellow]Usage Metadata:[/bold yellow]", str(usage_meta))

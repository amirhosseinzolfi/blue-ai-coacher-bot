...
def optimize_memory(state: AgentState) -> AgentState:
    ...
    # If a previous summary exists, exclude it from the new conversation text
    ...
    conversation_text = "\n".join([f"{msg.type}: {msg.content}" for msg in new_messages_for_summary])
    full_prompt = SUMMARY_PROMPT_TEXT.format(previous_summary=previous_summary, conversation_text=conversation_text)
    ...
    # Keep only the new summary and the last 2 messages from the new conversation
...
class MessageCounter:
    ...
    def increment_and_check(self, chat_id: str) -> bool:
        with self._lock:
            ...
            if self._counters[chat_id] >= 5:  # Summarize every 10 messages
...
def optimize_memory(state: State) -> State:
    """Enhanced memory optimization with better summarization triggers"""
    SUMMARY_THRESHOLD_MESSAGES = 8  # Summarize after 8 messages
    SUMMARY_THRESHOLD_TIME = 3600  # Summarize after 1 hour
    current_time = time.time()
    
    # Check if summarization is needed
    needs_summary = (
        state.message_count >= SUMMARY_THRESHOLD_MESSAGES or
        (current_time - state.last_summary_time) > SUMMARY_THRESHOLD_TIME
    )
    
    if not needs_summary:
        return state

    conv_messages = [msg for msg in state["messages"] 
                    if isinstance(msg, (HumanMessage, AIMessage))]
    
    if len(conv_messages) <= 2:  # Keep at least 2 messages
        return state
    ...
    summary_prompt = f"""Previous summary: {state.summary if state.summary else 'None'}
    
New conversation to summarize:
{conversation_text}

Create a concise summary focused on:
1. Key business decisions and tasks
2. Important user information
3. Critical context for future interactions
...
Summary:"""
    ...
    try:
        ...
        # Keep the system message + last 2 messages
        kept_messages = []
        if conv_messages and isinstance(conv_messages[0], SystemMessage):
            kept_messages.append(conv_messages[0])
        kept_messages.extend(conv_messages[-2:])
        
        # Create a system message with the summary
        summary_system_msg = SystemMessage(content=f"Previous Conversation Summary: {new_summary}")
        
        new_state = state.copy()
        new_state.messages = [summary_system_msg] + kept_messages
        new_state.summary = new_summary
        new_state.message_count = len(kept_messages)
        new_state.last_summary_time = current_time
        
        logger.info(f"Summarized conversation. Reduced from {len(conv_messages)} to {len(kept_messages)} messages")
        return new_state
...
def agent(state: State):
    """Enhanced agent with better context management"""
    chat_id = state["chat_id"]

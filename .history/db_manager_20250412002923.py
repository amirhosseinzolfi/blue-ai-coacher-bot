...
class DatabaseManager:
    ...
    def get_business_info(self, chat_id: str, chat_type: str = "private") -> str:
        ...
        try:
            ...
            info = doc.get("business_info", "") if doc else ""
            logger.debug(f"Retrieved business info for chat {chat_id}: Found={bool(info)}")
            return info
        except Exception as e:
            logger.error(f"Error retrieving business info for chat {chat_id}: {e}")
            return ""
    ...
    def load_chat_context(self, chat_id: str, chat_type: str = "private") -> Dict:
        ...
        try:
            ...
            logger.debug(f"Business info length: {len(business_info)}")
            logger.debug(f"Chat history entries: {len(chat_history)}")
            ...
            logger.error(f"Error loading chat context for {chat_id}: {e}")
            return {
                "business_info": "",
                "chat_history": [],
                "settings": {"ai_tone": "دوستانه", "language": "fa"}
            }
...
def load_chat_context(chat_id: str, chat_type: str = "private") -> Dict:
    """Load full context for a chat (or group) including business info, chat history, session metadata and settings."""
    context = {
        "business_info": _db_manager.get_business_info(chat_id, chat_type),
        "chat_history": _db_manager.get_chat_history(chat_id, chat_type),
        "metadata": _db_manager.get_chat_metadata(chat_id, chat_type),
        "settings": {
            "ai_tone": _db_manager.get_setting(chat_id, "ai_tone"),
            "language": _db_manager.get_setting(chat_id, "language"),
            "conversation_summary": _db_manager.get_conversation_summary(chat_id)
        }
    }
    return context

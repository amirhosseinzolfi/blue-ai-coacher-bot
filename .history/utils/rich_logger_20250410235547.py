...
def setup_logger(level=logging.INFO):
    ...
    logger.info("[bold cyan]🤖 Blue Business Bot Logger Initialized[/bold cyan]")
    logger.info("[bold green]💬 Logging level set to %s[/bold green]", logging.getLevelName(level))
...
def log_llm_request(logger, chat_id, session_id=None, ai_tone=None, prompt=None, history_count=None, user_context_available=False):
    """
    Log minimal and structured details of the LLM request.
    Displays: Chat ID, Session ID, AI Tone, Raw Prompt (shortened), History Count and User Context.
    """
    table = Table(title="[bold magenta]LLM Request Details[/bold magenta]", show_header=True, header_style="bold blue")
    ...
    if prompt:
        ...
        table.add_row("Raw Prompt", short_prompt)
    if history_count is not None:
        table.add_row("History Count", str(history_count))
    table.add_row("User Context", "Yes" if user_context_available else "No")
    
    console.print(table)
    logger.debug(f"LLM Request prepared for chat {chat_id}")

# New function to log AI interaction (minimized)
def log_ai_interaction(logger, prompt, response, model_name=None, duration=None):
    """
    Log minimal and structured AI interaction.
    Displays: Summarized Prompt and Response.
    """
    model_info = f" ({model_name})" if model_name else ""
    time_info = f" [{duration:.2f}s]" if duration else ""
    
    table = Table(title=f"[bold blue]AI Interaction{model_info}{time_info}[/bold blue]", show_header=True, header_style="bold blue")
    table.add_column("Component", style="cyan", no_wrap=True)
    table.add_column("Summary", style="white")
    
    sprompt = prompt if len(prompt) < 80 else prompt[:77] + "..."
    sresponse = response if len(response) < 80 else response[:77] + "..."
    table.add_row("Prompt", sprompt)
    table.add_row("Response", sresponse)

...
def close_client_session(session):
    # New helper to close a given client session with logging
    try:
        if session and hasattr(session, "close") and callable(session.close):
            session.close()
            logging.getLogger("blue_business").info("Client session closed successfully.")
    except Exception as e:
        logging.getLogger("blue_business").error(f"Error closing client session: {e}")

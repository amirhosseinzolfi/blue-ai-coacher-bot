...
def log_ai_interaction(logger, prompt, response, model_name=None):
    """Minimal AI interaction log"""
    logger.info("-" * 30)
    logger.info(f"Model: {model_name or 'Unknown'}")
    logger.info(f"In: {prompt[:50]}...")
    logger.info(f"Out: {response[:50]}...")
    logger.info("-" * 30)
...
def log_comprehensive_interaction(logger, chat_id, session_id, system_instruction, user_raw_prompt, user_report, conversation_summary, ai_tone=None):
    """Logs minimal interaction details"""
    logger.info("-" * 40)
    logger.info(f"Chat: {chat_id} | Session: {session_id[:8]}...")
    logger.info(f"Input: {user_raw_prompt[:50]}...")
    if ai_tone:
        logger.info(f"Tone: {ai_tone}")
    logger.info("-" * 40)

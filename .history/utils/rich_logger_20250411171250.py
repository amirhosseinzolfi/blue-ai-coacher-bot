...
def format_conversation_summary_table(messages, summary_text):
    """Format conversation summary in a rich table."""
    table = Table(
        title="[bold blue]Conversation Summary[/bold blue]",
        box=box.ROUNDED,
        show_header=True,
    ...
    table.add_column("Count/Content", style="yellow")
    
    # Message statistics
    human_msgs = len([m for m in messages if "Human" in str(m.type)])
    ai_msgs = len([m for m in messages if "AI" in str(m.type)])
    
    table.add_row("Total Messages", str(len(messages)))
    table.add_row("User Messages", str(human_msgs))
    table.add_row("AI Messages", str(ai_msgs))
    table.add_row("Summary", summary_text[:100] + "..." if len(summary_text) > 100 else summary_text)
    
    return table

def display_conversation_preview(messages, max_preview=5):
    """Display recent messages in a preview table."""
    preview = Table(
        title="[bold yellow]Recent Messages Preview[/bold yellow]",
        box=box.ROUNDED,
        show_header=True,
        header_style="bold magenta"
    )
    
    preview.add_column("Type", style="cyan", width=10)
    preview.add_column("Content", style="white", width=50)
    
    for msg in messages[-max_preview:]:
        msg_type = "👤 User" if "Human" in str(msg.type) else "🤖 AI"
        content = str(msg.content)
        if len(content) > 47:
            content = content[:47] + "..."
        preview.add_row(msg_type, content)
    
    return preview

# Add these functions to __all__ if you have it
__all__ = [
    # ...existing exports...
    'format_conversation_summary_table',

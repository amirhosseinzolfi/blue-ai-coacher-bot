...
def get_history_for_chat(telegram_chat_id: str, session_id: str = None):
    ...
    from db_manager import get_current_session
    
    if not session_id:
        session_id = get_current_session(telegram_chat_id)
        if not session_id:
            session_id = new_chat_session(telegram_chat_id)
...
def save_message_to_history(chat_id: str, role: str, content: str, session_id: Optional[str] = None):
    """Save message to specific session history."""
    from db_manager import get_current_session
    
    if not session_id:
        session_id = get_current_session(chat_id)
        if not session_id:
...
def new_chat_session(chat_id: str) -> str:
    """Creates a new chat session while preserving business info."""
    from db_manager import create_new_chat_session, save_message_to_history
    
    # Create new session
    new_session_id = create_new_chat_session(chat_id)
    
    # Save welcome message to new session
    welcome_text = "🆕 جلسه گفتگوی جدید آغاز شد. چطور می‌توانم به شما کمک کنم؟"
    save_message_to_history(chat_id, "system", welcome_text, session_id=new_session_id)

...
def new_chat(message):
    ...
    save_message_to_history(chat_id, "system", welcome_text)
...
def handle_daily_tasks(call):
    ...
    process_run_agent_async(f"{sender_first_name} : {prompt_input}", chat_id, placeholder_message.message_id, sender_first_name)
...
def handle_message(message):
    ...
    # Instead of direct blocking call, run the agent asynchronously.
    process_run_agent_async(query_payload, chat_id, placeholder_message.message_id, sender_first_name)
...
# Add helper function for asynchronous processing
def process_run_agent_async(query, chat_id, message_id, sender_first_name):
    import threading
    def target():
        try:
            refined_response = run_agent(query, chat_id, message_id, sender_first_name)
            ...
            logger.info(f"[bold green]✅ Response sent to {sender_first_name}[/bold green]")
        except Exception as e:

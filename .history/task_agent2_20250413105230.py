...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            # Structured logging for each input and LLM processing
            logging.info("=== New Input Received ===")
            logging.info("User Input: %s", user_input)

            try:
                response = llm.invoke(user_input)
                logging.info("=== LLM Response Start ===\n%s\n=== LLM Response End ===", response)
            except Exception as llm_err:
                logging.error("LLM invocation error: %s", llm_err)
                response = "Error calling agent. Please try again."
            print("Agent:", response)

            # Process commands with structured logging for each key step
            if 'add task' in user_input.lower():
                ...
                logging.info("Processed Command: Add Task -> '%s'", title)
                ...
                if task_manager.mark_task_done(task_title):
                    logging.info("Processed Command: Mark Task Done -> '%s'", task_title)
                    print(f'[INFO] Task marked as done: {task_title}')
                else:
                    logging.info("Processed Command: Mark Task Done -> Task Not Found: '%s'", task_title)
                ...
                if task_manager.refine_task(task_title, new_description):
                    logging.info("Processed Command: Refine Task -> '%s'", task_title)
                    print(f'[INFO] Task refined: {task_title}')
                else:
                    logging.info("Processed Command: Refine Task -> Task Not Found: '%s'", task_title)
                    print(f'[WARN] Task not found: {task_title}')
            elif 'summarize tasks' in user_input.lower():
                summary = task_manager.summarize_tasks()
                logging.info("Processed Command: Summarize Tasks")
                print("\n" + "="*40)
                print("TASKS SUMMARY:")
                print(summary)
                print("="*40 + "\n")

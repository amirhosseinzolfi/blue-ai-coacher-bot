...
def new_chat(message):
    ...
    # Determine chat type based on message type
    ...
    save_business_info(int_chat_id, {"user_report": ""}, chat_type)

    # Save the new session ID (function handles conversion)
    save_session_id(int_chat_id, new_session_id, chat_type)

    logger.info(f"New chat session created for chat '{int_chat_id}': {new_session_id} (Chat history reset, user report refreshed)")
    ...
    save_message_to_history(chat_id_str, "system", welcome_text, session_id=new_session_id, chat_type=chat_type)
...
def send_welcome(message):
    ...
    save_message_to_history(chat_id, "system", welcome_message, session_id=f"{chat_id}_default", chat_type=chat_type)
...
def send_help(message):
    ...
    save_message_to_history(chat_id, "system", help_text_prompt, session_id=f"{chat_id}_default", chat_type=chat_type)
...
def about_bot(message):
    ...
    save_message_to_history(chat_id, "system", about_text, session_id=f"{chat_id}_default", chat_type=chat_type)
...
def bot_settings(message):
    ...
    save_message_to_history(chat_id, "system", settings_text, session_id=f"{chat_id}_default", chat_type=chat_type)
...
def options_handler(message):
    ...
    save_message_to_history(chat_id, "system", options_text, session_id=f"{chat_id}_default", chat_type=chat_type)
...
def handle_daily_tasks(call):
    ...
    chat_type = "group" if call.message.chat.type in ['group', 'supergroup'] else "private"
    ...
    try:
        refined_response = run_agent(f"{sender_first_name} : {prompt_input}", chat_id, placeholder_message.message_id, sender_first_name, chat_type)
        save_message_to_history(chat_id, "assistant", refined_response, session_id=f"{chat_id}_default", chat_type=chat_type)
...
def handle_load_business_info(call):
    chat_id = str(call.message.chat.id)
    chat_type = "group" if call.message.chat.type in ['group', 'supergroup'] else "private"
    current_info = get_user_business_info(chat_id, chat_type)
    keyboard = telebot.types.InlineKeyboardMarkup()
    if current_info:
...
def handle_message(message):
    ...
    save_message_to_history(chat_id, "user", f"{sender_first_name}: {user_message_text}", session_id=f"{chat_id}_default", chat_type=chat_type)
    ...
    try:
        refined_response = run_agent(query_payload, chat_id, placeholder_message.message_id, sender_first_name, chat_type)
...
def handle_document(message):
    ...
    if business_info_update_pending.get(chat_id):
        ...
        if mode == "append":
            ...
            optimized = new_info
        save_user_business_info(chat_id, optimized, chat_type)
        business_info_update_pending.pop(chat_id, None)
        business_info_mode.pop(chat_id, None)
        bot.send_message(chat_id, "✅ اطلاعات بیزینس به‌روزرسانی شد.")
        bot.send_message(chat_id, f"🔍 اطلاعات بیزینس نهایی:\n{optimized}")

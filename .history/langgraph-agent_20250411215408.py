...
llm = ChatOpenAI(
    ...
    temperature=0.5,
    api_key="123"  # Changed from "324" to "123"
)
...
def extract_semantic_memory(messages):
    # Convert messages to dicts for LangMem (assumes HumanMessage/AIMessage have 'content')
    input_messages = []
    for m in messages:
    ...
    semantic_manager = create_memory_manager(
        ...
        enable_inserts=True,
        enable_deletes=True,
        api_key="123"  # Pass authentication for Anthropic
    )
    try:
        ...
        print(f"\n[Semantic Memory Extraction Error]: {e}")

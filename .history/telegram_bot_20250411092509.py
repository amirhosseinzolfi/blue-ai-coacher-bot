...
def optimize_memory(state: AgentState) -> AgentState:
    """
    Optimizes the conversation history by summarizing it if the number
    of human/assistant messages exceeds 10. The summary is generated 
    using only the last 10 messages and then inserted as a SystemMessage.
    """
    MESSAGE_THRESHOLD = 10
    logger.info(f"Entering optimize_memory for chat: {state['chat_id']}")
    
    # Filter human and assistant messages
    conv_messages = [msg for msg in state["messages"] if isinstance(msg, (HumanMessage, AIMessage))]
    if len(conv_messages) <= MESSAGE_THRESHOLD:
        logger.info(f"Conversation length ({len(conv_messages)}) does not exceed threshold ({MESSAGE_THRESHOLD}); skipping summarization.")
        return state

    # Build summarization prompt using previous summary if exists
    ...
    if existing_summary:
        ...
        summary_prompt_text = "Create a summary of the conversation below:"
    
    # Use only the last MESSAGE_THRESHOLD messages for the new summary
    messages_to_summarize = conv_messages[-MESSAGE_THRESHOLD:]
    ...
    # Create a new system message containing the updated summary
    new_system = SystemMessage(content=f"[CONVERSATION SUMMARY]: {new_summary}")
    # Replace the conversation history with the summary and the last MESSAGE_THRESHOLD messages
    new_messages = [new_system] + state["messages"][-MESSAGE_THRESHOLD:]
    logger.info(f"Trimmed conversation: kept {len(new_messages)} messages (summary + last {MESSAGE_THRESHOLD} messages)")
...
def agent(state: AgentState):
    ...
    try:
        ...
        # Replace placeholders with the user's name if needed
        if username:
        ...
        # Inline optimization of memory: if conversation history exceeds threshold, summarize it

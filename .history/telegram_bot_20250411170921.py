...
def format_conversation_preview(messages, max_length=50):
    """Format conversation messages for display in logs."""
    formatted = []
    for msg in messages:
...
def optimize_memory(state: AgentState) -> AgentState:
    ...
    human_msgs = [m for m in conv_messages if isinstance(m, HumanMessage)]
    ai_msgs = [m for m in conv_messages if isinstance(m, AIMessage)]
    
    stats_table = Table(title="[bold blue]Conversation Statistics[/bold blue]")
    stats_table.add_column("Metric", style="cyan")
    stats_table.add_column("Count", justify="right", style="green")
    stats_table.add_row("Total Messages", str(len(conv_messages)))
    stats_table.add_row("User Messages", str(len(human_msgs)))
    stats_table.add_row("AI Messages", str(len(ai_msgs)))
    ...
    # Show preview of messages being summarized
    preview_table = Table(title="[bold yellow]Messages Being Summarized (Preview)[/bold yellow]")
    preview_table.add_column("Type", style="cyan", width=12)
    preview_table.add_column("Content", style="white", width=60)
    
    for msg in format_conversation_preview(conv_messages[-5:]):  # Show last 5 messages
        msg_type, content = msg.split(" ", 1)
        preview_table.add_row(msg_type, content)
    ...
    try:
        ...
        logger.info("[bold blue]🔄 Generating new summary...[/bold blue]")
        ...
        summary_table = Table(title="[bold green]Summary Comparison[/bold green]")
        summary_table.add_column("Type", style="cyan")
        summary_table.add_column("Content", style="yellow", width=60)
        
        if existing_summary:
            summary_table.add_row("Previous Summary", existing_summary[:100] + "..." if len(existing_summary) > 100 else existing_summary)
        summary_table.add_row("New Summary", new_summary[:100] + "..." if len(new_summary) > 100 else new_summary)
        console.print(summary_table)
        
        # Show compression stats
        compression = (len(new_summary) / len(conversation_text)) * 100
        logger.info(f"[bold green]✨ Summarization complete in {duration:.2f}s[/bold green]")
        logger.info(f"[cyan]📊 Compression ratio: {compression:.1f}% of original size[/cyan]")
        ...
        logger.process_end("[red]Summarization failed[/red]")
        return state
    ...
    logger.info(f"[blue]Final message count: {len(new_messages)} (1 summary + {len(conv_messages[-2:])} recent messages)[/blue]")
    logger.process_end("[bold green]✨ Conversation optimization completed[/bold green]")
...
def run_agent(query, chat_id, message_id, username=None):
    ...
    try:
        with MongoDBSaver.from_conn_string(
            ...
            # Check if this is a user message and update counter
            should_summarize = message_counter.increment_and_check(chat_id, is_user_message=True)
            if should_summarize:
                logger.info(f"[yellow]🔄 Will trigger summarization for chat {chat_id}[/yellow]")

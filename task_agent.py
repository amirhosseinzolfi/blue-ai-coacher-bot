...
try:
    ...
    print("G4F API module imported successfully")
except ImportError:
...
def wait_for_api_server(timeout=30):
    ...
    print("[INFO] Waiting for the G4F API server to become available...")
    ...
    while True:
        try:
            ...
            if response.ok:
                print("[INFO] G4F API server responded successfully.")
                break
        except Exception:
            pass
            
        if (datetime.datetime.now() - start_time).seconds > timeout:
            print("[ERROR] API server not available after waiting 30 seconds.")
            return
        time.sleep(1)
...
from langgraph.checkpoint.sqlite import SqliteSaver  # Using SqliteSaver for checkpointing
...
# --- Timeout Helper for LLM Invocation ---
def invoke_with_timeout(llm_messages, timeout=30):  # Increased timeout to 30 seconds
    q = queue.Queue()
    def target():
        try:
            res = llm.invoke(llm_messages)
            q.put(res)
...
def save_tasks(tasks: List[Task]):
    try:
        with open(TASKS_FILE, 'w') as f:
            json.dump([task.model_dump() for task in tasks], f, indent=4)  # Changed from dict() to model_dump()
...
def analyze_request(state: AgentState):
    ...
    formatted_tasks = json.dumps([task.model_dump() for task in current_tasks], indent=2) if current_tasks else "No tasks yet."
    ...
    try:
        # Use the timeout helper for LLM invocation
        raw_ai_response = invoke_with_timeout(llm_messages, timeout=10)
        if raw_ai_response is None:
            print("LLM invocation timed out.")
            state['next_action'] = 'error'
            state['response'] = "Sorry, the response took too long. Please try again."
            return state

        response_content = raw_ai_response.content
        print(f"LLM Raw Response Content:\n{response_content}")  # Debug output

        # --- Extract JSON from potential Markdown block ---
        json_string = response_content
        match = re.search(r"```(?:tool_code|json)\s*(\{.*?\})\s*```", response_content, re.DOTALL | re.IGNORECASE)
        if match:
            ...
            if not (json_string.startswith('{') and json_string.endswith('}')):
                print("WARN: LLM response doesn't appear to be JSON or wrapped in ```tool_code/json```.")

        # --- Parse the extracted JSON string ---
        try:
            # Add a default 'response' field if missing
            parsed_json = json.loads(json_string)
            if 'response' not in parsed_json:
                parsed_json['response'] = "Action processed, but no response provided."
            
            ai_response = TaskAction.model_validate(parsed_json)
            print(f"LLM Parsed Result: action='{ai_response.action}', id={ai_response.task_id}, desc='{ai_response.task_description}', response='{ai_response.response}'")
        except Exception as json_e:
            print(f"Error parsing extracted JSON: {json_e}")
            print(f"Problematic JSON string: {json_string}")
            state['next_action'] = 'error'
            state['response'] = "Sorry, I couldn't understand the response format from the analysis. Please try rephrasing."
            return state
        ...
        print(f"Error during LLM analysis or response processing: {e}")
        state['next_action'] = 'error'
            ...
            try:
                ...
                for event in final_state_stream:
                    if isinstance(event, dict) and END in event:
                        ...
                        # Immediately extract response when we get the final state
                        if final_state_value:
                            if final_state_value.get('messages'):
                                last_message = final_state_value['messages'][-1]
                                if isinstance(last_message, AIMessage):
                                    assistant_response = last_message.content
                                else:
                                    assistant_response = final_state_value.get('response', 
                                        "Sorry, I didn't get a valid response.")
                            else:
                                assistant_response = final_state_value.get('response', 
                                    "Sorry, something went wrong during processing.")

                # Use captured response or fallback
                if assistant_response == "Processing...":
                    assistant_response = "Sorry, I didn't get a complete response. Please try again."

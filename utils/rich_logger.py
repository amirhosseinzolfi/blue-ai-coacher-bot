...
def setup_logger(level=logging.INFO, logger_name="blue_business"):
    ...
    logger = logging.getLogger(logger_name)  # Use the logger_name parameter
    logger.info("=" * 50)
    logger.info(f"Logger '{logger_name}' Started")
    logger.info("=" * 50)
...
def log_ai_interaction(logger, prompt, response, model_name=None):
    """Minimal AI interaction log - can be simplified if log_comprehensive_interaction is used"""
    logger.info(f"[purple]🤖 Model: {model_name or 'Unknown'}[/purple] | In: {prompt[:30]}... | Out: {response[:30]}...")
...
# New function to log comprehensive interaction details in a table
def log_comprehensive_interaction(logger, chat_id, session_id, system_instruction, user_raw_prompt, ai_response, ai_tone=None, model_name=None, history=None, summary=None):
    """Logs full interaction details including full history and summary."""
    
    table = Table(title=f"[bold blue]💬 Interaction Details (Chat: {chat_id} | Session: {session_id})[/bold blue]", 
                  show_header=True, header_style="bold magenta", border_style="blue")
    
    table.add_column("Component", style="cyan", no_wrap=True, width=18)
    table.add_column("Content", style="green")
    
    if model_name:
        table.add_row("Model", f"[purple]{model_name}[/purple]")
    if ai_tone:
        table.add_row("AI Tone", f"[magenta]{ai_tone}[/magenta]")
    
    # summary if provided (this is the raw summary string from state.summary)
    if summary is not None:
        table.add_row("Current Summary", summary) # Changed label for clarity
        table.add_row("Summary Length", str(len(summary)))
    
    # system instruction & its length
    table.add_row("System Instruction", system_instruction)
    table.add_row("SysInstr Length", str(len(system_instruction)))
    
    # history details (these are now just the conversation turns)
    if history is not None:
        table.add_row("History Turns Count", str(len(history))) # Renamed for clarity
        # join with newlines for readability
        hist_text = "\n".join([turn for turn in history]) # history is already list of strings
        table.add_row("History Messages", hist_text) # Renamed for clarity
    
    # user prompt & its length
    table.add_row("User Prompt", user_raw_prompt)
    table.add_row("UserPrompt Length", str(len(user_raw_prompt)))
    
    # ai response & its length
    table.add_row("AI Response", ai_response)
    table.add_row("AIResp Length", str(len(ai_response)))
    
    console.print(table)
    logger.info(f"[bold blue]Interaction logged for Chat {chat_id}[/bold blue]")
